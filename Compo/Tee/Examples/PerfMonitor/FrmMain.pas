...
type
  ...
  private
    ...
    procedure AddNewCounter(PathBuffer:TCounterPathBuffer; TimeInterval:Boolean=False);
...
procedure TMainForm.AddNewCounter(PathBuffer:TCounterPathBuffer; TimeInterval:Boolean=False);
var tmp : TCounterItem;
begin
  tmp:=TCounterItem.Create(Chart1,PathBuffer,TimeInterval);
...
begin
  ...
  AddNewCounter('\Processor(_Total)\% Processor Time');
  AddNewCounter('\LogicalDisk(_Total)\% Disk Time');
  AddNewCounter('\Memory\Pages/sec', True);
...
begin
  if ChartListBox1.ItemIndex=-1 then result:=nil
  else
