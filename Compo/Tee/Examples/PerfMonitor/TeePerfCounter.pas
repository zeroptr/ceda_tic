...
{ Copyright 2004-205 by David Berneda         }
{*********************************************}
unit TeePerfCounter;
{$I TeeDefs.inc}
...
type
  ...
  public
    ...
    Constructor Create(Chart:TCustomChart; Path:TCounterPathBuffer; TimeInterval:Boolean=False);
...
begin
  if Error <> ERROR_SUCCESS then
     raise Exception.Create('Error: ' + IntToHex(Error,8)+' '+SysErrorMessage(Error));
end;

Constructor TCounterItem.Create(Chart:TCustomChart; Path:TCounterPathBuffer; TimeInterval:Boolean=False);
begin
  FScale:=1;
...
begin
  ...
  Description.Clear;
  Description.Add(CounterInfo^.szExplainText);
