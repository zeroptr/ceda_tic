...
uses
  ...
  TeeToolsGalleryDemos, TeeWorldSeries;
...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure asTab1Click(Sender: TObject);
    procedure Hide3Click(Sender: TObject);
    procedure asWindow1Click(Sender: TObject);
...
Uses
     ...
     TeeStore,
     TeeSVGCanvas, TeePSCanvas, TeeXAMLCanvas,
     ...
     TeeBrushDlg, TeeEdiTitl, TeeAnnToolEdit, TeeVMLCanvas,
     TeeExpForm, TeeDragPoint, TeeSourceEdit,
     ...
     TeePolar, TeeSurfa, TeeTriSurface, TeePoin3,
...
begin { show the About... dialog }
  TeeShowAboutBox(TeeChartOfficeVersion,TeeChartOfficeVersion);
end;
...
begin { initialization when form is shown }
  ...
  { Note: This is done here instead of at FormCreate due to a VCL bug }
  if not AsTab1.Checked then asWindow1Click(Self);
  ...
  begin
    ...
    ImageList3.GetBitmap(1,CreateButton(0,Open1Click,TeeMsg_Open,'',0).Glyph);
    SetChildOrder(Controls[ControlCount-tmp],0);

    { create the "New" button }
    ImageList3.GetBitmap(0,CreateButton(0,New1Click,TeeMsg_New,'',0).Glyph);
    SetChildOrder(Controls[ControlCount-tmp],0);

    { change the "normal" icon }
    ImageList3.GetBitmap(32,ButtonNormal.Glyph);
...
begin { empty the current chart }
  ...
  try
    tmpEmpty.Title.Text.Add(TeeMsg_Chart1);
...
procedure TMainForm.FormCreate(Sender: TObject);
  ...
  begin { create and load program configuration }
    ...
    begin
      ...
      Add('Gallery','Visible',ScrollBox1);
      ...
      Gallery1.Checked:=ScrollBox1.Visible;
      if not RightSide1.Checked then ScrollBox1.Align:=alLeft;
...
begin { initialization }
  ...
  { Start at Chart tab }
  PageControl1.ActivePage:=TabChart;
...
begin { when closing the program, save the "recent files" to registry }
...
procedure TMainForm.ImportfromWeb1Click(Sender: TObject);
  ...
  Function DoSaveChartToStream:TMemoryStream;
  begin { return a Stream with current Chart contents }
    { disable things we dont want to be saved }
    Selector.ParentChart:=nil;
    DBChart1.OnDblClick:=nil;
    DBChart1.OnMouseMove:=nil;
    try
      ...
      SaveChartToStream(DBChart1,result);
    finally
      { reset internal events and Selector tool }
      Selector.ParentChart:=DBChart1;
      DBChart1.OnDblClick:=DBChart1DblClick;
  ...
  Procedure UploadChart(Const Email,Password,ChartID:String);
  var tmp : TMemoryStream;
  begin { send current Chart to Web Gallery }
    Screen.Cursor:=crHourGlass;
    try
      StatusHint.Text:=Format(TeeMsg_UploadingWeb,[ChartID]);
      StatusBar1.Update;
      try
        { upload }
        tmp:=DoSaveChartToStream;
        try
          {$IFNDEF CLX}
          TeeUploadFile(Format(TeeSteemaUploadGallery,[Email,Password,ChartID]),tmp);
          {$ENDIF}
        finally
          tmp.Free;
        end;
        ...
        Screen.Cursor:=crDefault;
        StatusHint.Text:='';
        ShowMessage(Format(TeeMsg_Uploaded,[ChartID]));
      finally
        StatusHint.Text:='';
      end;
    finally
...
begin { Show the Web Chart Gallery or load a Chart from a web URL pointing
  ...
  try
    ...
    if CurrentFile='' then
       EditChart.Text:=TeeMsg_Chart1
    else
       EditChart.Text:=ExtractFileName(CurrentFile);

    { calculate current Chart size in bytes }
    with DoSaveChartToStream do
    try
      LabelKB.Caption:=FormatFloat('0.#',Size/1024.0)+'KB';
    finally
      Free;
    end;
    ...
    begin
      ...
      begin { load chart from web address URL }
        ChartWebSource1.URL:=URL.Text;
        ImportChartFromWeb;
      end
      else
      if PanelUpload.Visible then
      begin { upload to Web Gallery }
        if TeeYesNo(TeeMsg_SureToUploadChart) then
           UploadChart(EditEmail.Text,EditPassword.Text,EditChart.Text);
      end
      else
        { go to TeeChart Gallery Web site home page }
        TeeGotoURL(Self.Handle,TeeSteemaWebGallerySite);
    end;
  finally
    Free;
  end;
...
begin { show / hide the chart gallery }
  ScrollBox1.Visible:=Toogle(Gallery1);
end;
...
begin { edit selected Annotation }
  ...
  try
    ...
    Align:=alNone;
    Position:=poScreenCenter;
    Tag:={$IFDEF CLR}Variant{$ELSE}Integer{$ENDIF}(Selector.Annotation);
    ShowModal;
    TeeModified;
  finally;
    Free;
...
begin { align the Chart gallery to Right or Left side }
  Toogle(RightSide1);
  if RightSide1.Checked then ScrollBox1.Align:=alRight
                        else ScrollBox1.Align:=alLeft;
end;
...
begin { draw selected point color at status bar }
  if Assigned(Selector) and
     ...
     (Selector.Part.PointIndex<>-1) then
...
procedure TMainForm.asTab1Click(Sender: TObject);
begin { show / hide the Chart Data tab }
  if Showing then
  begin
    TabData.Visible:=Toogle(asTab1);
    TabData.TabVisible:=TabData.Visible;
    Hide3.Checked:=False;
...
begin { hide Chart Grid and navigator }
  ...
  begin
    AsTab1.Checked:=False;
    AsWindow1.Checked:=False;
    TabData.Visible:=False;
    TabData.TabVisible:=TabData.Visible;
  end
  else
  if asWindow1.Checked then asWindow1Click(Sender)
                       else asTab1Click(Sender);
end;

procedure TMainForm.asWindow1Click(Sender: TObject);
begin { show Chart Grid as Window, below Chart }
  Toogle(AsWindow1);
  Hide3.Checked:=False;
...
begin { Set controls in Border toolbar }
  ...
  begin
    ...
    else ComboPenStyle.ItemIndex:=Ord(tmp.Style);
  end
  else
  begin
...
begin
  if Small1.Checked then ChartGalleryWidth:=50
                    else ChartGalleryWidth:=100;
  ScrollBox1.Width:=ChartGalleryWidth+17;  // 17 = scrollbar width
