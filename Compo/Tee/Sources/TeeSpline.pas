...
const MaxFragments            = 600; // The maximum of straight line segments allowed for drawing the spline
  ...
  // MaxInterpolatedVertices :Integer= 250; // The maximum number of vertices that can be interpolated, up to 16000 allowed
      ...
      MaxIterations           = 80;
      VerticesIncrement       = 5;  // Number of vertices to allocate memory for when the count property exceeds the current capacity
...
begin
  ...
  begin
    ...
    begin
      SetLength(FPointList,NewCapacity+1 {*SizeOf(TVertex)} );
...
begin
  ...
  Inc(FNoPoints);
  FPointList[FNoPoints].X:=X;
  FPointList[FNoPoints].Y:=Y;
  Build:=False;
end;
...
begin
  ...
  begin
    ...
    begin
      if dist<1 then
         mix:=FourSixths-dist*dist+0.5*dist*dist*dist
      else
      begin
        tmp:=2-dist;
        mix:=tmp*tmp*tmp*OneSixth;
      end;
...
begin
  ...
  begin
    ...
    // Improvement:  "if any of the ValuesLists has Order"...
    if not HasZValues then  // 7.03
    begin
