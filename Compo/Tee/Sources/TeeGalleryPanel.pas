...
type
  ...
  private
    ...
    function ClickedArrow(const P:TPoint):Boolean;
    function GetChart(Index:Integer):TGalleryChart;
  ...
  public
    ...
    class Function DefaultMode:Integer;
    class Function DefaultSmooth:Boolean;
    ...
    class Procedure SaveMode(Value:Integer);
    class Procedure SaveSmooth(Value:Boolean);

    procedure UseTheme(Theme: TChartThemeClass);

    property Chart[Index:Integer]:TGalleryChart read GetChart; default;
...
begin
  ...
  LeftAxis.Grid.SmallSpace:=3;
  LeftAxis.LabelsFont.Size:=7;
  LeftAxis.LabelsFont.Color:=clDkGray;

  BottomAxis.LabelStyle:=talValue;
  BottomAxis.Grid.SmallDots:=True;
  BottomAxis.LabelsFont.Size:=7;
  BottomAxis.LabelsFont.Color:=clDkGray;
  BottomAxis.Grid.SmallSpace:=3;
  ...
  {$IFDEF TEEANTIALIAS}
  Canvas:=TAntiAliasCanvas.Create;
  {$ENDIF}
end;
...
{$IFDEF LCL}
procedure Frame3D(Canvas:TCanvas; const R:TRect; const ColorA,ColorB:TColor; Size:Integer);
var tmp : TRect;
begin
  tmp:=R;
  Canvas.Pen.Color:=ColorA;
  Canvas.Frame3D(tmp,Size,bvLowered);
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    LeftAxis.LabelStyle:=tmp;
    BottomAxis.LabelStyle:=tmp;
  end;
end;
*)
...
begin
  ...
  begin
    ...
    TextFactor:=ScaleFactor div 2;

    with LeftAxis.LabelsFont do Size:=Size*TextFactor;
    with BottomAxis.LabelsFont do Size:=Size*TextFactor;
    ...
    with LeftAxis.LabelsFont do Size:=Size div TextFactor;
    with BottomAxis.LabelsFont do Size:=Size div TextFactor;
  end;
end;
...
begin
  result:=TeeReadBoolOption('SmoothGallery',False); // Do not localize
end;

class Function TChartGalleryPanel.DefaultMode:Integer;
begin
  result:=TeeReadIntegerOption('GalleryMode',0); // Do not localize
end;

class Procedure TChartGalleryPanel.SaveSmooth(Value:Boolean);  // 6.02
begin
  TeeSaveBoolOption('SmoothGallery',Value); // Do not localize
end;

class Procedure TChartGalleryPanel.SaveMode(Value:Integer);  // 6.02
begin
  TeeSaveIntegerOption('GalleryMode',Value); // Do not localize
end;
...
Var tmpClass : TChartSeriesClass;
  ...
  begin
    ...
    begin
      tmpSeries:=CreateNewSeries(nil,result,tmpClass,AType.FunctionClass);

      if AType.SubIndex<>0 then
         TSeriesAccess(tmpSeries).SetSubGallery(tmpSeries,AType.SubIndex);
    end;
  end;
...
begin
  ...
  begin
    ...
    tmpNum:=TeeNumFields(tmpDesc,TeeLineSeparator);

    for t:=1 to tmpNum do
        Title.Text.Add(TeeExtractField(tmpDesc,t,TeeLineSeparator));
...
begin
  ...
  begin
    ...
    if Assigned(FOnChangeChart) then
       OnChangeChart(Self);
  end;

  for t:=0 to Charts.Count-1 do
      if Charts[t]<>SelectedChart then
         Charts[t].UnFocus(FView3D);
end;
