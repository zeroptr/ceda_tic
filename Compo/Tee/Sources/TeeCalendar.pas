...
type
  ...
  published
    ...
    property Transparent stored True nodefault;
    property Visible;
  end;
  ...
  private
    ...
    function GetNextMonth: TSpeedButton;
    function GetNextVisible: Boolean;
    function GetPreviousMonth: TSpeedButton;
  ...
  public
    ...
    function ClickedDate(x,y:Integer; out ADate:TDateTime):Boolean;
    Function Month:Word;
    Procedure NextMonth;
    ...
    property NextMonthButton:TSpeedButton read GetNextMonth;
    property PopupMenu:TPopupMenu read GetPopupMenu;
    property PreviousMonthButton:TSpeedButton read GetPreviousMonth;
...
{$IFDEF LCL} // SysUtils does not export GetLocaleStr
...
begin
  ...
  IFirstDay:=0;
  {$ELSE}
  IFirstDay:=StrToIntDef(GetLocaleStr(GetThreadLocale, LOCALE_IFIRSTDAYOFWEEK, '0'), 0); // Do not localize
  {$ENDIF}
  {$ENDIF}
  ...
  FWeekDays.Pen.Hide;
  FWeekDays.Format:='ddd'; // Do not localize

  FMonths:=TCalendarCellUpper.Create(nil);
  FMonths.Parent:=Self;
  FMonths.Transparent:=True;
  FMonths.Format:='mmmm, yyyy'; // Do not localize
  ...
  // AddNull should be called at the end of this Constructor
  // to avoid undefined instance access at SetParentChart.
  AddNull;
...
Function TCalendarSeries.CreateButton(Const ACaption:String; ALeftPos:Integer):TSpeedButton;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      // Force recreating buttons:
      GetNextMonth;
      GetPreviousMonth;

      if Today.Color=clTeeColor then
         Today.Color:=ParentChart.GetDefaultColor(3);

      if Sunday.Color=clTeeColor then
         Sunday.Color:=ParentChart.GetDefaultColor(0);
    end;
  end;
end;
...
begin
  result:='TCalendarSeriesEditor'; // Do not localize
end;
...
var tmpDate : TDateTime;  // 7.05
begin
  if PointInRect(Months.ShapeBounds,X,Y) then
     with ParentChart.ClientToScreen(TeePoint(X,Y)) do
          PopupMenu.Popup(x,y) { 5.02 }
  else
  if ClickedDate(x,y,tmpDate) then // 7.05
     Date:=tmpDate;
end;

// Returns True when xy pixel coordinates are over a calendar cell containing
// a date.  "ADate" parameter returns the corresponding cell datetime.
function TCalendarSeries.ClickedDate(x,y:Integer; out ADate:TDateTime):Boolean;
...
begin
  ...
  if Assigned(FNextMonth) then
     FNextMonth.Visible:=Visible;

  if Assigned(FPreviousMonth) then
     FPreviousMonth.Visible:=Visible;
end;

function TCalendarSeries.GetNextMonth: TSpeedButton;
begin
  if not Assigned(FNextMonth) then
     FNextMonth:=CreateButton('>',30); // Do not localize

  result:=FNextMonth;
end;

function TCalendarSeries.GetPreviousMonth: TSpeedButton;
begin
  if not Assigned(FPreviousMonth) then
         FPreviousMonth:=CreateButton('<',6); // Do not localize

  result:=FPreviousMonth;
end;
