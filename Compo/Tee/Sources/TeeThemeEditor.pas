...
begin
  ...
  begin
    if CBPalette.ItemIndex=0 then
       ApplyChartTheme(SelectedTheme,PreviewChart)
    else
...
begin
  ...
  begin
    if CBPalette.ItemIndex=0 then
       ApplyChartTheme(SelectedTheme,Chart)
    else
       ApplyChartTheme(SelectedTheme,Chart,CBPalette.ItemIndex-1);
  end
  else
    Chart.ColorPaletteIndex:=CBPalette.ItemIndex-1;
...
begin
  ...
  if Assigned(Chart) then
     if Chart.ColorPaletteIndex=0 then
        CBPalette.ItemIndex:=0
     else
        CBPalette.ItemIndex:=Chart.ColorPaletteIndex+1;
...
begin
  if CBPalette.ItemIndex=0 then
     ...
     PreviewChart.ColorPaletteIndex:=CBPalette.ItemIndex-1;

  if CBScale.Checked then
     ScaledChart.Invalidate;
...
begin
  if CBScale.Checked then
  begin
    if not PreviewChart.Canvas.ReDrawBitmap then
       PreviewChart.Draw;
