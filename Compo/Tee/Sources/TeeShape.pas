...
begin
  ...
  Begin
    ...
    if Self.Brush.Color=clTeeColor then
       AssignBrush(Self.Brush,ParentChart.GetDefaultColor(SeriesIndex),Self.SeriesColor)
    else
       AssignBrush(Self.Brush,Self.SeriesColor);
...
begin
  ...
  begin
    ...
    if Brush.Color=Color then
       Color:=BrushColors[Color=clBlack];
...
Begin
  ...
  begin
    ...
    begin
      ...
      with ParentChart.Canvas do
      begin
        if tmp then DestRect:=CalcRect3D(R,MiddleZ)
               else DestRect:=R;

        tmpBlend:=BeginBlending(DestRect,Transparency);
      end
      else
         tmpBlend:=nil;
...
Begin
  ...
  begin
    ...
    if IsEnabled then Brush.Color:=clTeeColor
...
procedure TChartShape.SetTransparency(const Value: TTeeTransparency);
begin
