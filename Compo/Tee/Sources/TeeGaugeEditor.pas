...
type
  ...
  private
    ...
    function CanModifyAxis:Boolean;
  public
    { Public declarations }
  end;
...
function TGaugeSeriesEditor.CanModifyAxis:Boolean;
begin
  result:=Showing and (Gauge.Axis<>nil);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      with Axis do
      begin
        UDIncrement.Position:=Round(Increment);  // 7.0
    ...
    if not Assigned(tmpCircled) then
       tmpCircled:=TCircledSeriesEditor.CreateForm(Self,Gauge);
...
procedure TGaugeSeriesEditor.FormDestroy(Sender: TObject);
begin
