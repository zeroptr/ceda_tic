...
type
  ...
  protected
    ...
    function CalcMinMaxValue(IsMin:Boolean):TChartValue; virtual;
    ...
    procedure InitDrawValue(ValueIndex:Integer);
  ...
  public
    ...
    Function CalcBarBounds(ValueIndex:Integer):TRect; virtual; abstract;
  ...
  private
    ...
    procedure SetGradientBright(Value:Integer);
  ...
  published
    ...
    property GradientBright:Integer read FGradientBright write SetGradientBright default 0;
...
var tmpColor    : TColor;
   ...
   var
     ...
     begin
       ...
       begin
         ...
         begin
            tmpP4:=P4;
            Dec(tmpP4[0].X,Pen.Width);
            Dec(tmpP4[0].Y,Pen.Width);
            Dec(tmpP4[1].X,Pen.Width);
            Dec(tmpP4[1].Y,Pen.Width);
            Inc(tmpP4[2].X,Pen.Width);
            Inc(tmpP4[2].Y,Pen.Width);
            Inc(tmpP4[3].X,Pen.Width);
            Inc(tmpP4[3].Y,Pen.Width);

            tmpBlend:=ParentChart.Canvas.BeginBlending(RectFromPolygon(tmpP4,4),Transparency)
         end
         else
            tmpBlend:=nil;
         ...
         begin
            ...
            begin
               ...
               tmpBlend:=ParentChart.Canvas.BeginBlending(ParentChart.Canvas.RectFromRectZ(Rect(tmpPoint.X,tmpPoint.Y,
                     tmpPoint.Y+FLineHeight+Pen.Width,tmpPoint.X+FLineHeight+Pen.Width),EndZ+Pen.Width),Transparency);
            end
            else
              tmpBlend:=nil;
   ...
   var tmpDifX, tmpPenWidth : Integer;
       OldDarkColor         : TColor;
       tmpDark3D            : Boolean;
       tmpP                 : TFourPoints;
       tmpBlend             : TTeeBlend;
...
//              LinePen.Color:=OldDarkColor;
           ...
           begin
             ...
             if Transparency>0 then
             begin
               tmpPenWidth := LinePen.Width;
               tmpP[0]:=Canvas.Calculate3DPosition(tmpPoint,StartZ-tmpPenWidth);
               tmpP[1]:=Canvas.Calculate3DPosition(tmpOldP,StartZ-tmpPenWidth);
               tmpP[2]:=Canvas.Calculate3DPosition(tmpOldP,EndZ+tmpPenWidth);
               tmpP[3]:=Canvas.Calculate3DPosition(tmpPoint,EndZ+tmpPenWidth);
               tmpBlend:=BeginBlending(PolygonBounds(tmpP),Transparency);
             end
              else
                tmpBlend:=nil;
       ...
       begin // line 2D
         if (ColorEachPoint and not Pointer.Visible) or
            (ColorEachLine and Pointer.Visible) or (not Pointer.Visible) then
         ...
         begin
           tmpPenWidth := LinePen.Width;
           tmpBlend:=BeginBlending(rect(min(oldx,x)-tmpPenWidth,
                min(oldy,y)-tmpPenWidth,max(oldx,+x)+tmpPenWidth,
                max(oldy,y)+tmpPenWidth) ,Transparency);
         end
         else
           tmpBlend:=nil;
...
Begin
  ...
  with ParentChart.Canvas do
  Begin
    if ((IsNull(ValueIndex)) and (TreatNulls=tnSkip)) then
    ...
    if YMandatory then a:=GetHorizAxis
                  else a:=GetVertAxis;

    tmpLast:=LastDisplayedIndex;
    IsLastValue:=ValueIndex=tmpLast;
    ...
    Begin
      ...
      Begin  { previous point outside left }
        ...
        begin
          if a.Inverted then OldX:=ParentChart.ChartRect.Right
      ...
      if ValueIndex=tmpLast then //TV52013257
      begin
        DrawPoint(True);

        if ((tmpFirst=LastValueIndex) and (ValueIndex>0)) then // <- previous point outside left
        begin
          if drawArea then
          begin
            OldX:=CalcXPos(ValueIndex - 1);
            OldY:=CalcYPos(ValueIndex - 1);
            OldBottomPos:=GetOriginPos(ValueIndex - 1);
          end
          else
          begin
            if a.Inverted then OldX:=ParentChart.ChartRect.Right
            ...
            if stairs then
              ...
              OldY:=CalcYPosLeftRight(XScreenToValue(OldX), ValueIndex-1);
          end;
...
procedure TCustomSeries.InitDrawValue(ValueIndex:Integer);
begin
  ...
  begin
    ...
    OldY:=CalcYPos(ValueIndex-1);
    OldBottomPos:=GetOriginPos(ValueIndex-1);
    OldColor:=ValueColor[ValueIndex-1];
  end;
end;
...
function TCustomSeries.IsPointInChartRect(ValueIndex: Integer): Boolean;
var point   : TRect;
    r, tmpR : TRect;
begin
    point:=TeeRect(CalcXPos(ValueIndex) - Pointer.HorizSize, CalcYPos(ValueIndex) - Pointer.VertSize,
        CalcXPos(ValueIndex) + Pointer.HorizSize, CalcYPos(ValueIndex) + Pointer.VertSize);
...
procedure TCustomSeries.CalcFirstLastVisibleIndex;
begin
  inherited;
  while (FLastVisibleIndex < Count - 1) and IsPointInChartRect(FLastVisibleIndex + 1) do Inc(FLastVisibleIndex);
  while (FLastVisibleIndex > -1) and (FLastVisibleIndex < Count - 1) and (NotMandatoryValueList.Items[FLastVisibleIndex] = NotMandatoryValueList.Items[FLastVisibleIndex+1]) do Inc(FLastVisibleIndex);
  while (FFirstVisibleIndex > 0) and IsPointInChartRect(FFirstVisibleIndex - 1) do Dec(FFirstVisibleIndex);
  while (FFirstVisibleIndex > 0) and (NotMandatoryValueList.Items[FFirstVisibleIndex] = NotMandatoryValueList.Items[FFirstVisibleIndex-1]) do Dec(FFirstVisibleIndex);
end;
...
var oldLinePen   : TChartPen;
    oldOutLine   : TChartHiddenPen;
    tmpColor : TColor;
    tmpPColor : Boolean;
    tmpSize  : Integer;
begin
  ...
  if Assigned(OutLine) and OutLine.Visible then { 5.02 }
  begin
    oldLinePen:=TChartPen.Create(nil);
    oldOutLine:=TChartHiddenPen.Create(nil);
    try
      oldLinePen.Assign(LinePen);
      oldOutLine.Assign(OutLine);
      tmpColor:=SeriesColor;
      SeriesColor:=OutLine.Color;
      LinePen:=OutLine;
      LinePen.Width:=2*LinePen.Width+oldLinePen.Width;
...
Procedure TAreaSeries.GalleryChanged3D(Is3D:Boolean);
begin
  inherited;
  SetGalleryFormat;
end;
...
procedure TAreaSeries.SetGalleryFormat;
begin
...
Begin
  ...
  with ParentChart do
  for t:=0 to SeriesCount-1 do
    if Series[t] is Self.ClassType then //TV52013006
...
function TCustomBarSeries.IsPointInChartRect(ValueIndex: Integer): Boolean;
var r, tmpR : TRect;
begin
   r:=ParentChart.ChartRect;
   {$IFDEF CLX}
   result:=IntersectRect(tmpR,r,CalcBarBounds(ValueIndex));
   {$ELSE}
   result:={$IFNDEF CLR}Windows.{$ENDIF}IntersectRect(tmpR,r,CalcBarBounds(ValueIndex));
   {$ENDIF}
end;
...
Procedure TCustomBarSeries.InternalApplyBarMargin(var MarginA,MarginB:Integer);
  ...
  begin
    ...
    Begin
      ...
      if tmpAxis.Automatic then
         tmp:=IAxisSize div IMaxBarPoints
      else
      begin
        if Count > 0 then
           tmpInc:=(NotMandatoryValueList.MaxValue -
                    NotMandatoryValueList.MinValue) / Count
        else
           tmpInc:=0;

        tmp:=Abs(tmpAxis.CalcPosValue(NotMandatoryValueList.MaxValue + tmpInc) -
                 tmpAxis.CalcPosValue(NotMandatoryValueList.MinValue));

        tmp:=tmp div IMaxBarPoints;
      end;
...
begin
  ...
  begin
    FBarBounds:=CalcBarBounds(ValueIndex);
...
begin
  ...
  begin
    ...
    begin
      ...
      {$IFDEF CLR} //TV52013884
      if csDesigning in ParentChart.Owner.ComponentState then
...
begin
  ...
  begin
    ...
    if BarPen.Visible then
    begin
      Inc(tmpR.Right,BarPen.Width);
      Inc(tmpR.Bottom,BarPen.Width);
    end;

    R1:=OrientRectangle(ParentChart.Canvas.CalcRect3D(tmpR,StartZ));
    R2:=OrientRectangle(ParentChart.Canvas.CalcRect3D(tmpR,EndZ));
    UnionRect(R,R1,R2);
  end
  else
    R:=FBarBounds;
...
Function TBarSeries.CalcBarBounds(ValueIndex:Integer):TRect;
var Return : TRect;
...
Function THorizBarSeries.CalcBarBounds(ValueIndex:Integer):TRect;
var Return : TRect;
  function PenWidth:Integer;
  begin
...
function TCircledSeries.IsPointInChartRect(ValueIndex: Integer): Boolean;
begin
  result:=true;
end;
...
begin
  ...
  if Marks.Visible and IAdjustCircleForMarks then AdjustCircleMarks
...
Procedure TPieSeries.SetGradientColor(AColor:TColor);
begin
  CircleGradient.EndColor:=AColor;

  if GradientBright<>0 then
     if GradientBright>0 then
        CircleGradient.StartColor:=ApplyBright(AColor,GradientBright)
     else
        CircleGradient.StartColor:=ApplyDark(AColor,-GradientBright)
end;
...
begin
  ...
  if ParentChart.View3D and (IsExploded or (DonutPercent>0) or (AngleSize<>360))
  ...
  begin
    ...
    begin
      if (not IsExploded) and ((DonutPercent>0) or (AngleSize<>360)) then
...
procedure TPieSeries.SetGradientBright(Value:Integer);
begin
  SetIntegerProperty(FGradientBright,Value);
...
Begin
  ...
  begin
    ...
    begin
      ...
      if CircleGradient.Visible then
      begin
//        Gradient.Angle:=Round(FAngles[ValueIndex].MidAngle/TeePiStep);
        SetGradientColor(tmpColor);
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Stairs) then
        begin
          if (t>FirstValueIndex) and (PointInLine(P,tmpX,tmpY,tmpX,OldY) or PointInLine(P,tmpX,OldY,OldX,OldY))then
          begin
            result:=t-1;
            break;
          end;
