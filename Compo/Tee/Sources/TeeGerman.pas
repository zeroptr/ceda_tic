...
begin
  ...
  TeeMsg_AddNewGroup      :='Gruppe hizufügen';
  TeeMsg_ChangeGroupName  :='Gruppenamen ändern';
  ...
  // 7.0
  TeeMsg_View2D               :='2D';
  TeeMsg_Outline              :='Umriß';

  TeeMsg_Stop                 :='Stop';
  TeeMsg_Execute              :='Ausführen !';
  ...
  TeeMsg_SurfaceNearestTool   :='Flächennächster';
...
'SURFACE NEAREST=Nächster(Fläche)'#13+
'TIP-MARKS=Hinweis-Marker'#13+
'2D LIGHTING=2D-Beleuchtung'#13+
'ANNOTATION=Anmerkung'#13+
'ANTI-ALIAS=Antialias (Glätten)'#13+
'BANNER=Banner'#13+
'DATA TABLE=Datentabelle'#13+
'FADER=Fader'#13+
'FRAME=Rahmen'#13+
'FULL SCREEN=Bildsch.füllend'#13+
...
'Move the mouse over Series points=Maus über Reihenpunkte bewegen'#13+
'Series points display animated=Animierte Anzeige der Reihenpunkte'#13+
'Move mouse over Series points to display hints=Maus über Reihenpunkte bewegen um Hinweise zu anzuzeigen'#13+
'Several Color lines displayed at random positions=Zufallsverteile farbige Linien'#13+
'Color bands to fill axes background=Farbbänder füllen Achsenhintergrund'#13+
'Two bands fill axes grid lines space=Zwei Bänder füllen Achsengitter'#13+
'Click axes arrow to scroll=Achsen klicken zum scrollen'#13+
'Drag axes to scroll=Achsen ziehen zum scrollen'#13+
'Annotations that can be dragged and resized=Anmerkungen vergrößern und verschieben'#13+
'Move mouse over Surface to highlight cells=Zellen hervorheben: Maus darüber ziehen'#13+
'Click and drag Series points=Reihenpunkte klicken und verschieben'#13+
'Show additional Legend panels=zusätzliche Legenden anzeigen'#13+
'Click Gantt bars to drag and resize=Gantt-Balken klicken zum verschieben und vergrößern'#13+
'Move mouse over Pie slices=Maus über Kuchenstücke führen'#13+
'Statistics of Series data=Statisik der Reiehndaten'#13+
'Fill the region between two Series=Den Bereich zwischen zwei Reihen füllen'#13+
'Smooth line edges=Weiche Linienränder'#13+
'Click to select Chart items=Klicken um Chart-Elemente auszuwählen'#13+
...
'Fills region between Series and value=Bereich zwischen Reihe und Wert füllen'#13+
...
'Maximizes a Chart to full desktop size=Maximiert Chart auf Bildschrimgröße'#13+
'Adds 3D text annotations=Fügt eine 3D Text Anmerkung hinzu'#13+
'Magnify a Chart portion under the mouse=Chartauschnitt unter der Maus vergößern (Lupe)'#13+
...
'Press ESC key or Alt + F4 to reset=ESC-Taste oder Alt-F4 drücken zum Zurücksetzen'#13+
//Image Tool
...
'AREA LINES=Flächenränder'#13+
'AUTO SIZE=Grösse Auto'#13+
'AUTOSIZE=Grösse auto'#13+
'AXIS LINE=Achsenlinie'#13+
