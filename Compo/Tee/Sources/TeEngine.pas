...
{  Copyright (c) 1995-2009 by Steema Software }
{  All Rights Reserved                        }
{*********************************************}
unit TeEngine;
{$I TeeDefs.inc}
...
Const ChartMarkColor   = clInfoBk (* $80FFFF *);  { default Series Mark back color }
      ...
      TeeColumnSeparator: Char = #6;  // To separate columns in Legend
      TeeLineSeparator  : Char = #13; // To separate lines of text
...
type
  ...
  public
    ...
    // Renamed from ToString to ValueToString (RAD 2008)
    Function ValueToString(const Index:Integer):String;

    {$IFNDEF CLR}
    {$IFNDEF D12}
    // Compatibility with pre RAD 2008:
  ...
  protected
    ...
    OnCalcMaxLabel : TAxisOnGetNextLabel; // internal event, used by TAxisBreaksTool class
    OnDrawAxis     : TNotifyEvent;
    OnDrawGrids    : TAxisDrawGrids;

    Function AxisRect:TRect;
    Procedure DrawGrids(NumTicks:Integer);
    procedure DrawLineSegment(AStart,AEnd:Integer);
  ...
  {$IFDEF D9}
  TSeriesEnumerator={$IFDEF D11}record{$ELSE}class{$ENDIF}
  private
    FIndex: Integer;
  ...
  public
    ...
    function MoveNext: Boolean; // cannot be inlined due to usage inside "while" 
  ...
  private
    ...
    Procedure SetXValue(Index:Integer; Const Value:TChartValue);
    Procedure SetYValue(Index:Integer; Const Value:TChartValue);
    ...
    procedure SetDefaultNullValue(const Value: Double);
  protected
    ...
    Function IsPointInChartRect(ValueIndex:Integer):Boolean; virtual;
  ...
  public
    ...
    property DefaultNullValue:Double read FDefaultNull write SetDefaultNullValue;
    ...
    begin
      ...
      begin
        ...
        if Logarithmic then
         ...
         if (IRange>=1.1) and // 8.02 1.1 = 1 + 2*margins (ie: Line Pen.Width)
            AnySeriesHasLessThan(MaxNumLabels) then // MS : 7.04
...
var tmp : Integer;
begin
  if (MaxLabelSize>0) and (IAxisSize>0) then  // TV52013138
  begin
...
begin
  ...
  begin
    if Abs(IMinimum)>High(Int64)  then tmpMin:=Increment*IMinimum //NCR TV52013949
                                  else tmpMin:=Increment*Round(IMinimum);
...
Begin
  ...
  if Assigned(OnCalcMaxLabel) then
  begin
    tmpStop:=False;
    tmpGap:=IMaximum;
    OnCalcMaxLabel(Self,-1,tmpGap,tmpStop);
    IMaximum:=IMaximum-tmpGap;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        result:=0;
        limit := 5000;
        repeat  // TV52011721  modified 23_7_08 MM see bug report
          result:=Math.Max(result,MultiLineTextWidth(LabelValue(tmpB),tmpNum));
          tmpB:=tmpB-tmp;
          dec(limit);
        until (tmpB<tmpA) or (limit=0);
      end
      else
        result:=Math.Max(MultiLineTextWidth(LabelValue(tmpA),tmpNum),
                         MultiLineTextWidth(LabelValue(tmpB),tmpNum));

      result:=Canvas.TextWidth(' ')+result;

      FOnGetAxisLabel:=OldGetAxisLabel;
    end;
  end;

  Function MaxAxisTextWidth:integer;
  var
      tt, tmp, tmpResult, tmpSeries :integer;
      tmpLabel : String;
  begin
      tmpResult:=0;
      ...
      for tmpSeries:=0 to SeriesCount-1 do
        With Series[tmpSeries] do
          ...
          begin
            tmpLabel := Labels[tt];
            ParentChart.FOnGetAxisLabel(TChartAxis(Self),nil,-1,tmpLabel);
            tmpResult:=Max(tmpResult,ParentChart.MultiLineTextWidth(tmpLabel,tmp));
          end;
...
begin
  if Items.Automatic then
    Case CalcLabelStyle of
      ...
      talText  :
        begin
          if ((ParentChart <> nil) and (Assigned(ParentChart.FOnGetAxisLabel))) then
              result:=MaxAxisTextWidth()  // TF02013509
          else
              result:=ParentChart.MaxTextWidth;
        end;
    else
    {talNone : } result:=0;
    end
...
var tmpSt2 : String;
    tmpZ   : Integer;
    {$IFDEF AXUNICODE} wideSt, tmpWideSt, tmpWideSt2 : WideString; {$ENDIF}
  ...
  begin
    ...
    if (Pos('@U#',St) = 1) then
    Begin
      wideSt:=Unicoding.ConvertCPList2Char(Unicoding.ConvertUTF82CPList(St))
    end
    else
      wideSt:=Unicoding.StringToWideString(St,0);

    i:=Unicode.WideCharPos(wideSt,WideChar(TeeLineSeparator),1);
    while i>0 do
    begin
      Inc(result);
      Delete(WideSt,1,i);
...
begin
 ...
 0,360: begin
  ...
  {$IFDEF AXUNICODE}
    if (Pos('@U#',tmpSt) = 1) then
    Begin
      tmpWideSt:=Unicoding.ConvertCPList2Char(Unicoding.ConvertUTF82CPList(tmpSt))
    end
    else
      tmpWideSt:=Unicoding.StringToWideString(tmpSt,0);
  {$ENDIF}
  ...
  begin
    {$IFDEF AXUNICODE}
    i:=Unicode.WideCharPos(tmpWideSt,WideChar(TeeLineSeparator),1);
    if i>0 then tmpWideSt2:=Copy(tmpWideSt,1,i-1)
           else tmpWideSt2:=tmpWideSt;

    if Angle=0 then
    begin
      y:=y+Delta;
      if FLabelsExponent then DrawExponentLabel
                         else ParentChart.Canvas.TextOut3D(X,Y,tmpZ,Unicoding.CodeUTF8String(tmpWideSt2));
    end
    else
    begin
      ...
      ParentChart.Canvas.RotateLabel3D(X,Y,tmpZ,Unicoding.CodeUTF8String(tmpWideSt2),Angle);
    end;

    Delete(tmpWideSt,1,i);
    {$ELSE}
...
procedure TChartAxis.DrawLineSegment(AStart,AEnd:Integer);
...
begin
  ...
  begin
    ...
    MoveTo3D(PosAxis,tmp,AStart);
    LineTo3D(PosAxis,tmp,AEnd);
  end
  else
  begin
    if Horizontal then
       if OtherSide then
          // Top axis
          HorizMode(AStart,AEnd,PosAxis)
       else
          // Bottom axis
          HorizMode(AStart-CalcWallSize(LeftAxis),
                    AEnd+CalcWallSize(RightAxis),PosAxis+IWallSize)
    else
    begin
      if OtherSide then tmp:=IWallSize
                   else tmp:=-IWallSize; //-(Axis.Width div 2)+1;

      case FAxis.LineMode of
        lmLine: ParentChart.Canvas.VertLine3D(PosAxis+tmp,AStart,
                                   AEnd+CalcWallSize(BottomAxis),IZPos);
    lmCylinder: ParentChart.Canvas.Cylinder(True,PosAxis+tmp-Pen.Width,
                  AStart,PosAxis+tmp+Pen.Width,AEnd+CalcWallSize(BottomAxis),
                  IZPos-Pen.Width,IZPos+Pen.Width,True);
      else
        ParentChart.Canvas.Cube(PosAxis+tmp-Pen.Width,
                  PosAxis+tmp+Pen.Width,AStart,AEnd+CalcWallSize(BottomAxis),
...
var tmpValue    : Double;
  ...
  begin
    ...
    IWallSize:=ParentChart.CalcWallSize(Self);
    ...
    begin
      ...
      if Assigned(OnDrawAxis) then
         OnDrawAxis(Self)
      else
         DrawLineSegment(IStartPos,IEndPos);
    end;
    ...
    Begin
      ...
      begin
        ...
        begin
          ...
          tmpGap:=tmpValue;
          OnCalcMaxLabel(Self,-1,tmpGap,tmpStop);
          tmpGap:=tmpValue+tmpGap;
          OnCalcMaxLabel(Self,-1,tmpGap,tmpStop);
          tmpValue:=tmpValue+tmpGap;
        end;
...
// Renamed from ToString to ValueToString (RAD 2008)
function TChartValueList.ValueToString(const Index: Integer): String;
begin
...
{$IFNDEF D12}
// Compatibility with pre RAD 2008:
Function TChartValueList.ToString(const Index:Integer):String;
begin
  result:=ValueToString(Index);
end;
{$ENDIF}
{$ENDIF}
...
procedure TSeriesMarks.SetFontSeriesColor(const Value: Boolean);
begin
  ParentSeries.SetBooleanProperty(FFontSeriesColor,Value);
end;
...
var i     : Integer;
    tmpSt : String;
    {$IFDEF AXUNICODE} WideSt, tmpWideSt, tmpWideOut : WideString; {$ENDIF}
begin
  ...
  begin
    ...
    begin
      ...
      Repeat
        tmpWideOut := Copy(tmpWideSt,1,i-1);
        DrawTextLine(Unicoding.CodeUTF8String(tmpWideOut));
        tmpWideSt:=Copy(tmpWideSt,i+1,Length(tmpWideSt));
        Inc(tmpNumRow);
        i:=Unicode.WideCharPos(tmpWideSt,WideChar(TeeLineSeparator),1);
      Until i=0;

      if tmpWideSt<>'' then
         DrawTextLine(Unicoding.CodeUTF8String(tmpWideSt));
    end
    else
      DrawTextLine(St);
    {$ELSE}
...
procedure TChartSeries.ReadData(Stream: TStream);
  ...
  begin
    ...
    {$IFDEF D12WIN32}
    if L=255 then // <-- 255 = Unicode string flag
    begin
      Stream.Read(LInt, SizeOf(LInt));

      SetLength(Bytes,LInt);
      Stream.Read(Bytes[0],LInt);

      result:=TEncoding.Default.GetString(Bytes,0,LInt);
    end
    else
    begin
      // Old version, non-Unicode strings:

      SetString(tmpSt, nil, L);
      Stream.Read(Pointer(tmpSt)^, L);
      result:=tmpSt;
    end;
...
procedure TChartSeries.WriteData(Stream: TStream);
  ...
  begin
    ...
    LByte:=255;
    Stream.Write(LByte,SizeOf(LByte));

    Bytes:=TEncoding.Default.GetBytes(AString);
    LInt:=Length(Bytes);
    Stream.Write(LInt,SizeOf(LInt));
    Stream.Write(Bytes[0],LInt);
  end;
  {$ELSE}
...
Procedure TChartSeries.CalcFirstLastVisibleIndex;
  ...
  begin
    ...
    begin
        if ((MaximumOffset <> 0) or (MinimumOffset <> 0)) then  //CDI TF02013319
        begin
            if A>C then
                if Inverted then
                   result:=CalcPosPoint(C - MaximumOffset)
                else
		               result:=CalcPosPoint(A - MaximumOffset)
            else
                if Inverted then
                   result:=CalcPosPoint(C + MinimumOffset)
                else
                   result:=CalcPosPoint(A + MinimumOffset);
        end
        else
            if Inverted then
               result:=CalcPosPoint(C)
            else
               result:=CalcPosPoint(A);
    end
    else
    With GetVertAxis do
    begin
        if ((MaximumOffset <> 0) or (MinimumOffset <> 0)) then
        begin
            if B>D then
                if Inverted then
                   result:=CalcPosPoint(B + MinimumOffset)
                else
                   result:=CalcPosPoint(D + MinimumOffset)
            else
                if Inverted then
                   result:=CalcPosPoint(B - MaximumOffset)
                else
                   result:=CalcPosPoint(D - MaximumOffset);
        end
        else
            if Inverted then
                result:=CalcPosPoint(B)
            else
                result:=CalcPosPoint(D);
    end;
  end;
...
Begin
  ...
  Begin
    ...
    begin
      ...
      if NotMandatoryValueList.First <= tmpMin then  //TF02013701
      begin
      ...
      Begin
        ...
        Begin
          ...
          //TF02013811
          While (FLastVisibleIndex < tmpLastIndex) and (NotMandatoryValueList.Value[FLastVisibleIndex]<tmpMax) do
          begin
            Inc(FLastVisibleIndex);
          end;

          while (not DrawBetweenPoints) and 
             (FLastVisibleIndex > 0) and
             (NotMandatoryValueList.Value[FLastVisibleIndex]>tmpMax) do
...
// Returns the index of the series point that is
// displayed last when the series is drawn.
Function TChartSeries.LastDisplayedIndex:Integer;
begin
...
begin
  result:=Add(DefaultNullValue,ALabel,clNone);
end;
...
begin
  BeginUpdate;
  try
    result:=High(XValues)-Low(XValues)+1;
    for t:=Low(XValues) to High(XValues) do AddXY(XValues[t], YValues[t]);
  finally
    EndUpdate;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      if (Self.Owner<>nil) and
         (Self.Owner.ClassName = 'TContourSeries') Then
         //special case TV52013279
      Begin
          AssignColors;
          Self.Labels.Assign(Labels);
          Self.AssignValues(TChartSeries(Source));
      end
      else if (DataSource=nil) or ManualData then // 7.04  TV52010358
      begin
...
Procedure TChartSeries.SetYValue(Index:Integer; Const Value:TChartValue);
Begin
...
Procedure TChartSeries.SetXValue(Index:Integer; Const Value:TChartValue);
Begin
...
Function TChartSeries.IsPointInChartRect(ValueIndex:Integer):Boolean;
var r : TRect;
   point : TPoint;
begin
   point:=TeePoint(CalcXPos(ValueIndex), CalcYPos(ValueIndex));
   r:=ParentChart.ChartRect;
   result:=PointInRect(r, point);
end;
...
procedure TChartSeries.SetDefaultNullValue(const Value: Double);
begin
  FDefaultNull:=Value;
end;
...
var i : Integer;
    {$IFDEF AXUNICODE} wideSt : WideString; {$ENDIF}
begin
  result:=0;
  NumLines:=0;
  ...
  if (Pos('@U#',s) = 1) then
  Begin
    wideSt:=Unicoding.ConvertCPList2Char(Unicoding.ConvertUTF82CPList(s))
  end
  else
    wideSt:=Unicoding.StringToWideString(s,0);

  i:=Unicode.WideCharPos(wideSt,WideChar(TeeLineSeparator),1);
  while i>0 do
  begin
    result:=Math.Max(result,Canvas.TextWidth(Unicoding.CodeUTF8String(Copy(WideSt,1,i-1))));
    Inc(NumLines);
    Delete(WideSt,1,i);
    i:=Unicode.WideCharPos(WideSt,WideChar(TeeLineSeparator),1);
  end;
  ...
  if WideSt<>'' then
  begin
    result:=Math.Max(result,Canvas.TextWidth(WideSt));
    Inc(NumLines);
  end;
  {$ELSE}
  ...
  Begin
    ...
    begin
      ...
      begin
        ...
        if not IHideWhenVisible then // 9.0
        begin
  ...
  Procedure CalcSeriesRect;
    ...
    var tmpR : TRect;
      ...
      begin
        ...
        if tmpHasOffsets and Axis.Labels then //TV52013795
        with Axis do
             FPosTitle:=InflateAxisPos(FPosLabels,SizeLabels);
      end;
...
begin
  ...
  begin
    ...
    if not (ArrowHead=ahSolid) then //TF02013794
