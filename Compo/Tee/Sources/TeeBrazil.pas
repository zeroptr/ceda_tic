...
begin
  ...
  TeeMsg_SeriesSetDataSource:='Sem gráfico mestre para validar fonte de dados';
  TeeMsg_SeriesInvDataSource:='Sem fonte de dados válida: %s';
  ...
  TeeMsg_MaxPointsPerPage   :='Pontos máximos por página devem ser >= 0';
  ...
  TeeMsg_WarningHiColor     :='16K Cores ou mais requeridas para melhor apresentação';
  ...
  TeeMsg_RefreshInterval    :='Intervalo de atualização deve estar entre 0 e 60';
  TeeMsg_SeriesParentNoSelf :='Gráfico pai das séries não pode ser o mesmo!';
  ...
  TeeMsg_SureToApply        :='Aplicar Mudanças ?';
  ...
  TeeMsg_SeriesList             :='Lista de Séries';
  ...
  TeeMsg_CanNotFindTempPath    :='Impossível encontrar pasta Temporária';
  TeeMsg_CanNotCreateTempChart :='Impossível criar gráfico Temporário';
  TeeMsg_CanNotEmailChart      :='Impossível enviar e-mail para TeeChart: %d';
  ...
  TeeMsg_GalleryStats        :='Estatísticas.';

  TeeMsg_CannotFindEditor    :='Impossível achar formulário de edição de Séries: %s';
  ...
  TeeMsg_ActionPrintHint        :='Visualiza impressão do Gráfico';
  ...
  TeeMsg_SmithReactance       :='Reatância';
  ...
  {OCX 5.0.4}
  {$IFDEF TEEOCX}
  TeeMsg_ActiveXVersion      := 'Versão ActiveX ' + AXVer;
  ...
  TeeMsg_DatasourceError     := 'Fonte de dados não é uma série ou set gravado';
...
'SIZE %=% Tam'#13+
'SERIES DATASOURCE TEXT EDITOR=Editor de fonte de dados da série'#13+
'FIELDS=Campos'#13+
...
'NON DATABASE CHART=Gráfico Sem Banco de Dados'#13+
...
'EXPORT DIALOG=Exportar Diálogo'#13+
