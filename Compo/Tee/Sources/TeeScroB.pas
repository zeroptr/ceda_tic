...
type
  ...
  public
    ...
    Procedure RecalcPosition(ScrollAxes:Boolean=True);
  published
...
function TChartScrollBar.HasPaging:Boolean;
begin
  result:=FChart.Pages.MaxPointsPerPage>0;
end;
...
var tmpPos : Integer;
  ...
  procedure DoScrollAxes;
  begin
    if (not FChart.IPanning.Active) then
...
begin
  ...
  if Assigned(FOldOnPageChange) then
    FOldOnPageChange(FChart);
end;
...
begin
  ...
  if Assigned(FOldOnUndoZoom) then
    FOldOnUndoZoom(FChart);
end;
...
begin
  ...
  try
    ...
    if not Axis.Automatic then
       Axis.SetMinMax(OldMin,OldMax);
  end;
end;

{ Change the scroll bar position and thumb size }
Procedure TChartScrollBar.RecalcPosition(ScrollAxes:Boolean=True);
  ...
  begin
    if (FAxis=sbBoth) or (FAxis=sbDefault) then
       RecalcAxis(Axis1);

    if (FAxis=sbBoth) or (FAxis=sbOther) then
       RecalcAxis(Axis2);
  end;

begin
  IScrollAxes:=ScrollAxes;
  try
