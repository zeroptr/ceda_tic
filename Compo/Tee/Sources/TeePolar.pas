...
type
  TCustomPolarSeries=class;

  TOnGetPolarLabel=procedure(Sender:TCustomPolarSeries; const Angle:Double;
                             Index:Integer; var Text:String) of object;
  ...
  private
    ...
    FOnGetCircleLabel : TOnGetPolarLabel;
    ...
    function CalcValueColor(ValueIndex:Integer):TColor;
    Procedure CalcXYPos( ValueIndex:Integer;
                         ARadius:Integer; Var X,Y:Integer);
  ...
  protected
    ...
    Procedure CalcXYPosition(const XValue,YValue:TChartValue; ARadius:Integer; var X,Y:Integer);
    ...
    Procedure DrawPolarCircle(HalfWidth,HalfHeight,Z:Integer); virtual;
  ...
  public
    ...
    { events }
    property OnGetCircleLabel:TOnGetPolarLabel read FOnGetCircleLabel write FOnGetCircleLabel;
...
var tmpValue     : Double;
  ...
  Begin
    ...
    begin
      ...
      if Axis.Grid.Color=clTeeColor then
         AssignVisiblePenColor(Axis.Grid,clGray)
      else
  ...
  Procedure DrawXGrid;
    ...
    begin
      ...
      begin
        ...
        if Assigned(FOnGetCircleLabel) then
           FOnGetCircleLabel(Self,Angle,AIndex,tmpSt);
...
type
  TPanelAccess=class(TCustomAxisPanel)
  {$IFDEF CLR}
  public
    procedure DoDrawBitmap(Rect:TRect; Z:Integer);
  end
  {$ENDIF};

{$IFDEF CLR}
procedure TPanelAccess.DoDrawBitmap(Rect:TRect; Z:Integer);
begin
  DrawBitmap(Rect,Z);
end;
{$ENDIF}
  ...
  begin
    ...
    Begin
      ...
      begin
        ...
        TPanelAccess(ParentChart).{$IFDEF CLR}DoDrawBitmap{$ELSE}DrawBitmap{$ENDIF}(R,Width3D);
...
Procedure TCustomPolarSeries.CalcXYPos( ValueIndex:Integer;
                                        ...
                                        Var X,Y:Integer);
begin
  CalcXYPosition(XValue[ValueIndex],YValues.Value[ValueIndex],ARadius,X,Y);
end;
...
procedure TCustomPolarSeries.InternalDrawValue(ValueIndex,X,Y:Integer);
  ...
  begin
    ...
    begin
      ...
      ParentChart.SetBrushCanvas(CalcValueColor(ValueIndex),Brush,Brush.Color);
...
begin
  ...
  if ValueIndex=FirstValueIndex then
     MoveTo3D(X,Y,StartZ)  { <-- first point }
  else
  begin
...
begin
  ...
  begin
    if TreatNulls=tnDontPaint then
    begin
      tmp:=ValueIndex+1;
      if tmp=Count then
         tmp:=0;
      ...
      OldX:=CalcXPos(tmp);
      OldY:=CalcYPos(tmp);
      ParentChart.Canvas.MoveTo3D(OldX,OldY,StartZ);
    end;
  end
  else
     DoDraw;
...
function TCustomPolarSeries.CalcValueColor(ValueIndex:Integer):TColor;
begin
  result:=ValueColor[ValueIndex];

  if (result=clNone) and (TreatNulls=tnIgnore) then
     if ColorEachPoint then
...
begin
  ...
  if Pointer.Visible then
     ...
     begin
       if (TreatNulls=tnIgnore) or (not IsNull(t)) then
       begin
         tmpColor:=CalcValueColor(t);

         FPointer.PrepareCanvas(ParentChart.Canvas,tmpColor);
         Pointer.Draw(CalcXPos(t),CalcYPos(t),tmpColor,DoGetPointerStyle(t));
       end;
     end;
end;

function TCustomPolarSeries.DoGetPointerStyle(ValueIndex:Integer):TSeriesPointerStyle;
begin
...
begin
  inherited;
  if Brush.Style=bsClear then Pen.Color:=AColor;
end;
...
Function TCustomPolarSeries.Clicked(x,y:Integer):Integer; // 7.07 improved
var Old  : TPoint;
    P    : TPoint;
    tmp  : TPoint;
    tmpCenter : TPoint;

  function ClickedSegment:Boolean;
  begin
    result:=((Brush.Style<>bsClear) and PointInTriangle(P,tmpCenter,tmp,Old)) or
            (Pen.Visible and PointInLine(P,tmp,Old));
  end;
...
begin
  ...
  begin
    ...
    tmpCenter.X:=CircleXCenter;
    tmpCenter.Y:=CircleYCenter;
    ...
    begin
      ...
      if (tmp.X=X) and (tmp.Y=Y) then { clicked right on point }
      begin
        result:=t;
        break;
      end
      else
      if Pointer.Visible and  // 8.03  and (DoGetPointerStyle(t)<>psNothing
         (Abs(tmp.X-X)<Pointer.HorizSize) and
         (Abs(tmp.Y-Y)<Pointer.VertSize) then
      begin
        result:=t;
        break;
      end
      else
      if (t>FirstValueIndex) and ClickedSegment then
      begin
        result:=t-1;
        break;
      end;
    ...
    if (result=TeeNoPointClicked) and CloseCircle and (LastValueIndex>FirstValueIndex) then
    begin
      Old.X:=CalcXPos(FirstValueIndex);
      Old.Y:=CalcYPos(FirstValueIndex);

      if ClickedSegment then
         result:=LastValueIndex;
    end;
  end;
end;

Function TCustomPolarSeries.LegendItemColor(LegendIndex:Integer):TColor;
begin
  result:=CalcValueColor(LegendIndex);
end;

Procedure TCustomPolarSeries.DrawLegendShape(ValueIndex:Integer; Const Rect:TRect);
var tmpColor : TColor;
    tmp      : TSeriesPointerStyle;
begin
  if Pen.Visible then
  begin
    LinePrepareCanvas(ValueIndex);
  ...
  begin
    ...
    begin
      tmpColor:=CalcValueColor(ValueIndex);

      if Assigned(FOnGetPointerStyle) then
      begin
        Pointer.Color:=tmpColor;
        tmp:=FOnGetPointerStyle(Self,ValueIndex);
        tmpColor:=Pointer.Color;
      end;
    end;

    TeePointerDrawLegend(FPointer,tmpColor,Rect,Pen.Visible,tmp);
  end
  else
...
procedure TCustomPolarSeries.SetClockWiseLabels(const Value: Boolean);
  ...
  var i: Integer;
  begin
    for i:=0 to XValues.Count-1 do XValues[i]:=360-XValues[i];
  end;
begin
...
procedure TCustomPolarSeries.SetTreatNulls(const Value:TTreatNullsStyle);
begin
...
Procedure TCustomPolarSeries.CalcXYPosition(const XValue,YValue:TChartValue;
                                            ARadius:Integer; var X,Y:Integer);
...
begin
  ...
  if GetVertAxis.Inverted then
     tmpDif:=GetVertAxis.Maximum-YValue
  else
     tmpDif:=YValue-GetVertAxis.Minimum;
  ...
  begin
    ...
    AngleToPos(piDegree*XValue,tmpRadius,tmpRadius,X,Y);
  end;
end;
...
Function TRadarSeries.GetxValue(ValueIndex:Integer):TChartValue;
begin
...
begin
  if (TreatNulls=tnIgnore) or (not IsNull(ValueIndex)) then
  begin
