  ...
  begin
    ...
    begin
      ...
      begin
        if UpDown2.Position>3 then
           result:=result+DupeString('0',UpDown2.Position-3)+ThousandSeparator+'000'
        else
           result:=result+'#'+ThousandSeparator+DupeString('#',3-UpDown2.Position)+DupeString('0',UpDown2.Position);
      end
      else
    ...
    if UpDown1.Position>0 then
       if CBFixedDecimals.Checked then
          result:=result+DecimalSeparator+DupeString('0',UpDown1.Position)
       else
          result:=result+DecimalSeparator+DupeString('#',UpDown1.Position);
...
begin
  ...
  begin
    ...
    if i>0 then
       Delete(Format,i,Length(CurrencyString));

    i:=Pos(ThousandSeparator,Format);
    CBThousands.Checked:=i>0;
    if i>0 then
       Delete(Format,i,Length(ThousandSeparator));

    i:=Pos(DecimalSeparator,Format);
    if i>0 then
    begin
      SetIntegerRB;
      ...
      Delete(Format,i,tmp+Length(ThousandSeparator));
    end;
...
begin
  ...
  try
    ...
    RBDate.Visible:=AllowDates;
    CBDate.Visible:=AllowDates;
