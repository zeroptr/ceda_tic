...
Uses
  ...
  Variants,
  {$ENDIF}
  DBChart, TeeProcs, DB, TeeConst, Series;

const
  TeeSharp='#'; // Do not localize;
...
begin
  ...
  try
    ...
    Caption:=TeeMsg_DBSumEditCaption;
    PanelButtons.Show;
...
procedure TDBChartSumEditor.BApplyClick(Sender: TObject);
  ...
  begin
    Case Group of
      dgHour   : result:='HOUR'; // Do not localize
      dgDay    : result:='DAY'; // Do not localize
      dgWeek   : result:='WEEK'; // Do not localize
      dgWeekDay: result:='WEEKDAY'; // Do not localize
      dgMonth  : result:='MONTH'; // Do not localize
      dgQuarter: result:='QUARTER'; // Do not localize
      dgYear   : result:='YEAR' // Do not localize
    else
      result:='';
    end;
  end;
...
begin
  ...
  Case CBAgg.ItemIndex of
    0: tmp:='#SUM#'; // Do not localize
    1: tmp:='#COUNT#'; // Do not localize
    2: tmp:='#HIGH#'; // Do not localize
    3: tmp:='#LOW#'; // Do not localize
    4: tmp:='#AVG#'; // Do not localize
  else
       tmp:='';
  end;

  // Pending: Remove all series that belong to us, before adding a new one
  if (not Assigned(TheSeries)) and Assigned(Chart) then
     TheSeries:=Chart.AddSeries(TBarSeries);
  ...
  if CBGroup.ItemIndex<>-1 then
  begin
    tmpSt:=CBGroup.CurrentItem;

    if CBTimeStep.Enabled and (CBTimeStep.ItemIndex<>-1) then
       tmpSt:=TeeSharp+TeeDBGroupToStr(TTeeDBGroup(CBTimeStep.ItemIndex))+
              TeeSharp+tmpSt
    else
    case CBSort.ItemIndex of
      2: begin
           ...
           if CBSortType.ItemIndex=0 then tmpSt:='#SORTASC#'+tmpSt // Do not localize
                                     else tmpSt:='#SORTDES#'+tmpSt; // Do not localize
         end;
      1: begin
...
begin
  result:=(ASeries.DataSource is TDataSet) and
          (Copy(ASeries.MandatoryValueList.ValueSource,1,1)=TeeSharp);
end;
