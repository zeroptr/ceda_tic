...
type
  ...
  TXMLSourceEditor = class(TBaseSourceEditor)
    ...
    procedure EFileEnter(Sender: TObject);
  private
...
uses {$IFNDEF LINUX}
     ...
     {$ENDIF}
     {$ENDIF}
     TeeProcs, TeeProCo, TeePenDlg, TeeStringsEditor
     {$IFDEF AXUNICODE}
     ,TeeUnicode
     {$ENDIF}
     ;
      ...
      begin
        ...
        begin
          ...
          if VarIsClear(tmpText) then tmpTex:=''
                       {$IFDEF AXUNICODE}
                       else tmpTex:=Unicoding.CodeUTF8String(tmpText.NodeValue);
                       {$ELSE}
...
procedure TXMLSourceEditor.EnableApply;
begin
  BApply.Enabled:=True;
end;
...
begin
  ...
  try
    if (EFile.Text<>'') or (Memo1.Lines.Count>0) then
    ...
    if Assigned(DataSource) and (DataSource.XML.Count>0) then
...
begin
  ...
  DataSource.XML:=Memo1.Lines;
  DataSource.SeriesNode:=CBSeries.CurrentItem;
...
procedure TXMLSourceEditor.RBManualClick(Sender: TObject);
begin
  RBFile.Checked:=False;
  Memo1.SetFocus;
end;

procedure TXMLSourceEditor.RBFileClick(Sender: TObject);
begin
  RBManual.Checked:=False;
  EFile.SetFocus;
end;

procedure TXMLSourceEditor.Button1Click(Sender: TObject);
begin
  if TeeEditStrings(Self,Memo1.Lines) then
  begin
    EnableApply;
    Memo1Enter(Self);
    Memo1.SetFocus;
...
procedure TXMLSourceEditor.Memo1Change(Sender: TObject);
begin
  EnableApply;
end;

procedure TXMLSourceEditor.Memo1Enter(Sender: TObject);
begin
  RBFile.Checked:=False;
  RBManual.Checked:=True;
end;

procedure TXMLSourceEditor.EFileEnter(Sender: TObject);
begin
  RBFile.Checked:=True;
  RBManual.Checked:=False;
end;
