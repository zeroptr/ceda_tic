...
Uses
  TeeBrushDlg, TeeBarEdit, TeeConst, TeeEdiSeri;
...
begin
  ...
  begin
    ...
    if RGStyle.Visible then
       RGStyle.ItemIndex:=Ord(ErrorStyle);
...
begin
  if Showing then
     ErrorSeries.ErrorWidth:=UDBarWidth.Position;
end;

procedure TErrorSeriesEditor.BPenClick(Sender: TObject);
begin
  With ErrorSeries do
  if not (ErrorSeries is TErrorBarSeries) then
     SeriesColor:=ErrorPen.Color;
end;
...
procedure TErrorSeriesEditor.FormDestroy(Sender: TObject);
begin
  FreeAndNil(FBarForm);
end;
