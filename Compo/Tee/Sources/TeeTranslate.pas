...
Procedure EndMissing(Const Path,FileName:String);
...
Procedure EndExtract(const Path:String);

var Extract:TStringList=nil;
{$ENDIF}

Function TeeTranslateString(const Text:String):String;
...
begin
  ...
  Missing.SaveToFile(Path+'\'+FileName);

  MissingHelp.Add('');
  MissingHelp.Add('Maximum helpcontext = '+IntToStr(MaxHelpContext));
  MissingHelp.SaveToFile(Path+'\teemissinghelp.txt');
...
Procedure EndExtract(const Path:String);
begin
  Extract.SaveToFile(Path+'\teeextract.txt');
  Extract.Free;
end;
...
begin
 hotKeyStr:='';
 result:=Text;
 TeeCanTranslate(result,hotKeyStr);
end;
...
Procedure TeeTranslateAControl(AControl:TControl; const ExcludedChilds:Array of TControl);
 ...
 begin
  ...
  function IsExcludedChild(AComponent:TComponent):Boolean;
  var t : Integer;
  begin
    for t:=0 to Length(ExcludedChilds)-1 do
    if AComponent=ExcludedChilds[t] then
    begin
      result:=True;
      Exit;
    end;
...
begin
  ...
  begin
    ...
    if not IsExcludedChild(Components[t]) then
    begin
      { traverse all Components, not only Controls }
      ...
      if Components[t] is {$IFDEF CLR}TListBox{$ELSE}TCustomListBox{$ENDIF} then
      with {$IFDEF CLR}TListBox{$ELSE}TCustomListBoxAccess{$ENDIF}(Components[t]) do
      begin
        if Sorted then
        begin
          tmp:=-1;

          // Fix: Multiselect sorted listbox can return ItemIndex=0 when
          // Count is also zero.

          if (ItemIndex=-1) or (ItemIndex>=Items.Count) then
             tmpSt:=''
          else
             tmpSt:=Items[ItemIndex]
        end
        else
        begin
          tmp:=ItemIndex;
      ...
      if Components[t] is TPopupMenu then
      with TPopupMenu(Components[t]) do
      begin
        for tt:=0 to Items.Count-1 do
        begin
          {$IFDEF TEEFINDMISSING}
          CheckExtract(AControl.Components[t],Items[tt].Caption);
          {$ENDIF}
...
begin
  ...
  if LBLangs.ItemIndex=-1 then
     if LBLangs.Items.Count>0 then
        LBLangs.ItemIndex:=0; { default = English }
end;
...
initialization
  ...
  TeeProcs.TeeTranslateStringHook:=TeeTranslate.TeeTranslateString;
finalization
