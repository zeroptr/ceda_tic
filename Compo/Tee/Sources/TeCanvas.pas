...
var
  ...
  {$IFDEF TEEOCX}
  ColorPickerClosed     : Boolean=True;
  {$ENDIF}
...
type
  ...
  private
    ...
    procedure SetRegion(Value:TFilterRegion);
  protected
    AllowRegion : Boolean;
  ...
  published
    ...
    property Region:TFilterRegion read GetRegion write SetRegion;
  end;
  ...
  published
    property Filters:TFilterItems read GetFilters write SetFilters
     ...
     protected
       ...
       Function GetIsNoBMPGrid:Boolean; virtual; abstract;
     ...
     public
       ...
       Procedure GradientFill( Const Rect:TRect;
                               ...
                               Balance:Integer=50;
                               RadialX:Integer=0;
                               RadialY:Integer=0); virtual; abstract;
       ...
       property IsNoBMPGrid:Boolean read GetIsNoBMPGrid;
     ...
     public
       ...
       FIsNoBMPGrid     : Boolean;  // bitmap avoidance TGLCanvas, PDFCanvas
...
begin
  if Value > 100 then
    SetIntegerProperty(FPerspective,100)
  else
...
begin
  ...
  if Assigned(ABrush.Bitmap) {$IFDEF CLX}and (not ABrush.Bitmap.Empty){$ENDIF} then
  begin
    Brush.Style:=bsClear;
...
// Printer Bug workaround:  "var" instead of "const" Points array parameter
function InternalCreateRegion(tmpDC:TTeeCanvasHandle; var Points:Array of TPoint;
                              NumPoints:Integer):HRgn;
begin
  LPToDP(tmpDC,Points,NumPoints);
  result:=CreatePolygonRgn(Points,NumPoints,ALTERNATE);
end;
...
{$ENDIF}
  ...
  function PointArrayOf(const Points: Array of TPoint):TPointArray; {$IFDEF CLR}unsafe;{$ENDIF}
  var t : Integer;
  begin
...
begin
  ...
  tmpPoints:=PointArrayOf(Points);
  try
    Region:=QRegion_create(@tmpPoints,False);
  ...
  begin
    tmpPoints:=PointArrayOf(Points);
    try
      Region:=InternalCreateRegion(tmpDC,tmpPoints,NumPoints);
    finally
      tmpPoints:=nil;
    end;
  end
...
Var tmpRect : TRect;
  ...
  begin
    ...
    try
      tmp.FDirection:=gdRadial;
      tmp.FStartColor:=StartColor;
      tmp.FEndColor:=EndColor;

      tmp.FBalance:=Balance;
      tmp.FRadialX:=RadialX;
      tmp.FRadialY:=RadialY;
...
var Points      : Array[0..2*MaxCircleSteps+1] of TPoint;
  ...
  var t,tt : Integer;
      ...
      {$ENDIF}
  begin
    if not assigned(FBrush.Bitmap) then
...
Function TTeeCanvas3D.GetIsNoBMPGrid:Boolean;
begin
  result:=False;
end;
...
Procedure TTeeCanvas3D.SetBitmap(ABitmap:TBitmap);
begin
  FBitmap:=ABitmap;
end;
...
begin
  result:=Math.Max(Abs(HorizSize),Abs(VertSize));
end;
...
const
  WeightFactor={$IFDEF TEEMULDIV} 1000 {$ELSE} 100000 {$ENDIF};
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Red := r div WeightSum;
          Green := g div WeightSum;
          Blue := b div WeightSum;
        end
      end;
    end;
  end;
...
{          if y >= h then
        ...
        if WeightSum<>0 then // TV52012992
        begin
          if Assigned(Back) then
          begin
            ...
            begin
              ...
              Red:=(((Red*2*r) div WeightSum)+255) div 510;
              Green:=(((Green*2*g) div WeightSum)+255) div 510;
              Blue:=(((Blue*2*b) div WeightSum)+255) div 510;

              {
              Red:=(MulDiv(Red*2,r,WeightSum)+256) div 512;
              Green:=(MulDiv(Green*2,g,WeightSum)+256) div 512;
              Blue:=(MulDiv(Blue*2,b,WeightSum)+256) div 512;
              }

              {$ELSE}
              if (r=g) and (r=b) then
              begin
                InvWeightSum:= r*Inv255/WeightSum;

                Red  :=Round(InvWeightSum * Red);
                Green:=Round(InvWeightSum * Green);
                Blue :=Round(InvWeightSum * Blue);
              end
              else
              begin
...
var
  ...
  procedure DrawSmoothUsingLines;
  var t : Integer;
  begin
    if RoundSize>0 then
       DrawRoundRect
...
begin
  ...
  begin
    ...
    begin
      ...
      if ACanvas.SupportsFullRotation then
         DrawAsRectangle
      else
         DrawSmoothUsingLines;
    end
    else
      DrawAsRectangle;
...
begin
  ...
  begin
    ...
    if Direction=gdRadial then
       Canvas.GradientFill(Rect,tmpStart,tmpEnd,Direction,Balance,RadialX,RadialY)
    else
...
Begin
  ...
  if (GetDefaultFontName='Arial') and // Do not localize
     ...
     {$ELSE} // Delphi 2005,2006,2007,...
       GetDefaultFontName:='‚l‚r ‚oƒSƒVƒbƒN';
     {$ENDIF}
  {$ENDIF}
end;
...
begin
  tmp:=GetSymbolColor;
  {$IFDEF TEEOCX}               {TA05010881}
  if ColorPickerClosed then
  begin
...
procedure TTeeFilter.SetRegion(Value:TFilterRegion);
begin
