...
begin
  result:='TDragPointToolEdit';  // Do not localize
end;
...
begin
  if Assigned(Tool) and Assigned(Tool.Series) then
  with Tool.Series do
       Cursor:=TeeSetCursor(Cursor,CBCursor.CurrentItem);
end;

class function TDragPointTool.LongDescription: String;
begin
  result:=TeeMsg_DragPointDesc;
end;
