...
procedure TeeShowSeriesEditor( ADesigner: {$IFDEF D6}IDesigner{$ELSE}IFormDesigner{$ENDIF};
                               ...
                               EditSeriesProc:TTeeEditSeriesProc;
                               AGroup: TSeriesGroup=nil);
var tmp : TTeeSeriesDesigner;
begin
  if not Assigned(AList.Owner.Designer) then
  begin
    tmp:=TTeeSeriesDesigner.Create;
    AList.Owner.Designer:=tmp;
  ...
  with TTeeSeriesDesigner(AList.Owner.Designer).FEditor do
  begin
    ChartListBox1.Chart:=(AList.Owner as TCustomChart);
    ChartListBox1.SeriesGroup:=AGroup;
    UpdateCaption;
    Show;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(SeriesGroup) then
       NewCaption:=NewCaption+' '+SeriesGroup.Name;
  end
  else NewCaption:='';
...
begin
  ...
  if Assigned(tmp) then
     if Assigned(ChartListBox1.SeriesGroup) then
     begin
       ChartListBox1.SeriesGroup.Add(tmp);
       ChartListBox1.UpdateSeries;
     end;
