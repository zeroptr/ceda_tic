...
type
  ...
  TTeeTranslateStringHook=function(const Text:String):String;

var
  TeeTranslateHook:TTeeTranslateHook=nil;
  TeeTranslateStringHook:TTeeTranslateStringHook=nil;
...
Procedure TCustomTeePanel.Draw(UserCanvas:TCanvas; Const UserRect:TRect);
  ...
  begin
    ...
    begin
      ...
      begin
        if HorizSize>0 then
           Dec(ChartRect.Right, 2*HorizSize)
        else
           Inc(ChartRect.Left, -2*HorizSize);

        if VertSize>0 then
           Dec(ChartRect.Bottom, 2*VertSize)
        else
           Inc(ChartRect.Top, -2*VertSize);
      end;
    end;
...
var Rect : TRect;
  ...
  begin
    ...
    if Shadow.HorizSize>0 then
       ...
       Inc(Rect.Left,-2*Shadow.HorizSize+1);

    if Shadow.VertSize>0 then
       Dec(Rect.Bottom,2*Shadow.VertSize+1)
    else
       Inc(Rect.Top,-2*Shadow.VertSize+1);
...
begin
  ...
  {$IFDEF D12}
    ...
    AStream.Write(Pointer(ByteOrderMark)^,Length(ByteOrderMark));
  {$ENDIF}

  {$IFDEF CLR}
  Bytes := BytesOf(AsString);
  AStream.Write(Bytes, Length(Bytes));
  {$ELSE}
  tmpSt:=AsString;
  AStream.Write(Pointer(tmpSt)^,Length(tmpSt)*SizeOf(Char));
  {$ENDIF}
end;
...
begin
  ...
  result:=((((A1*X3-B1*Y3-C1>0) xor (A1*X4-B1*Y4-C1>0)) and
          ((A2*X1-B2*Y1-C2>0) xor (A2*X2-B2*Y2-C2>0))) or //TF02013228
          (((A1*X3-B1*Y3-C1<>0) xor (A1*X4-B1*Y4-C1<>0)) and
          ((A2*X1-B2*Y1-C2<>0) xor (A2*X2-B2*Y2-C2<>0)))); // TV52010079,TV52010196
end;
...
function TeeTranslateString(const Text:String):String;
begin
  if Assigned(TeeTranslateStringHook) then
     result:=TeeTranslateStringHook(Text);
end;
