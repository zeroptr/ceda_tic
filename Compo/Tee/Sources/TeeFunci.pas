...
begin
  result:='TAverageFuncEditor'; // Do not localize
end;
...
begin
  result:='TCustomFunctionEditor'; // Do not localize
end;
...
begin
  ...
  begin
    ...
    try
      for t:=FirstIndex to LastIndex do
          if IncludeNulls or (not SourceSeries.IsNull(t)) then
             AddValue(SourceSeries.MandatoryValueList.Value[t],t-FirstIndex); // 7.06
...
initialization
  ...
  RegisterTeeBasicFunction( TMedianTeeFunction,   {$IFNDEF CLR}@{$ENDIF}TeeMsg_FunctionMedian);
  RegisterTeeBasicFunction( TModeTeeFunction,     {$IFNDEF CLR}@{$ENDIF}TeeMsg_FunctionMode);
end.
