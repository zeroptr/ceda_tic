...
type
  ...
  published
    ...
    property RightWall;
    property ScrollMouseButton; // 7.03
    property SubFoot;
    property SubTitle;
    property Title;
...
function TListOfDataSources.IndexOf(ADataSet: TDataSet): Integer;
var t : Integer;
begin
  for t:=0 to Count-1 do
      if DataSource[t].DataSet=ADataSet then
      begin
        result:=t;
        exit;
      end;
...
Var HasAnyDataSet : Boolean;
  ...
  var tmpxLabel : String;
    ...
    begin
      ...
      {$ELSE}
      {$IFDEF LCL} // LCL does not support fkAggregate
      result:=AField.AsFloat;
      {$ELSE}
      With AField do
    ...
    begin
      ...
      begin
        ...
        begin
          tmpField:=ADataSet.FieldByName(tmpName);
          tmpMand:=GetFieldValue(tmpField);

          // 7.01 Should nullity be checked here or should be optional ?
    ...
    Begin
      if GroupPrefix=dgNone then
         if Assigned(LabelField) then
                                 {$IFDEF AXUNICODE}
                                 tmpXLabel:=Unicoding.CodeUTF8String(WideString(LabelField.AsVariant))
                                 {$ELSE}
                                 tmpXLabel:=LabelField.DisplayText
                                 {$ENDIF}
                                 else tmpXLabel:=''
      else
      begin
      ...
      if (NumFields=1) and  // 7.01 Do not check for nullity in "Single Record" mode
         Assigned(MandaField) and MandaField.IsNull then
            tmpColor:=clNone
      else
      {$ENDIF}
         tmpColor:=clTeeColor;
