...
begin
  ...
  for i:=0 to SeriesCount-1 do
    if Assigned(Series[i].Datasource) then
      {$IFDEF DATABASE}
      if (Series[i].DataSource is TTeeADOQuery) then
         Series[i].Datasource.Free
      else
      {$ENDIF}
        Series[i].Datasource:=nil;
...
Procedure TCustomChart.DoZoom(Const topi,topf,boti,botf,lefi,leff,rigi,rigf:Double);
  ...
  begin
    ...
    begin
      if Zoom.Direction<>tzdHorizontal then // Bug found and fixed by customer
      begin
...
Begin
  ...
  if Zoom.Animated then
     DoZoomAnimated; { animation... }
...
Procedure TCustomChartLegend.DrawLegend;
  ...
  begin
    ...
    {$ELSE}
    ;
    if Assigned(Symbol.FOnDraw) Then
    {$ENDIF}
...
var XLegendText  : Integer;
  ...
  var
    ...
    begin
      if (not Vertical) and Inverted then
...
var
  ...
  begin
    ...
    if Symbol.Visible and (Symbol.Position=spLeft) then
    begin
      XLegendColor:=tmp;
      XLegendText :=XLegendColor+IColorWidth+TeeLegendOff4+TeeLegendOff2;

      tmpShadow:=Abs(Symbol.Shadow.HorizSize);

      if Symbol.Shadow.Visible and (tmpShadow<>0) then
      begin
        Inc(XLegendText,tmpShadow);

        if Symbol.Shadow.HorizSize<0 then
           if Symbol.Position=spLeft then
              Inc(XLegendColor,tmpShadow);
      end;
    end
    else
    begin
      if not Symbol.Visible then
         Inc(tmp,TeeLegendOff2);  // Margin
  ...
  var tmpTotalWidth : Integer;
    ...
    begin
      ...
      begin
        ...
        begin
          Inc(tmpTotalWidth,TeeLegendOff2);

          if Symbol.Shadow.Visible then
             Inc(tmpTotalWidth,Abs(Symbol.Shadow.HorizSize));
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      if Symbol.Shadow.VertSize>tmpH then
         Inc(tmp,Symbol.Shadow.VertSize-tmpH);
    end;
  ...
  begin
    ...
    begin
      ...
      if ParentChart.Shadow.Visible then
         if ParentChart.Shadow.VertSize<0 then
            tmp:=-2*ParentChart.Shadow.VertSize;
