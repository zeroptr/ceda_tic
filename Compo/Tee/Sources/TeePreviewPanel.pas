...
begin
  ...
  try
    ...
    begin
      ...
      try
        TeeSetBitmapSize(tmpSmooth,R.Right-R.Left,R.Bottom-R.Top);
...
Begin
  if (Panel<>Value) and (Value<>Self) then  // 7.01
  begin
    if FPanels.Count>0 then
       FPanels.Delete(0);
    ...
    if Assigned(Panel) then
       FAsBitmap:=Panel.Canvas.SupportsFullRotation;
...
Begin
  ...
  begin
    ...
    if (result<>teePrev_None) and (not FAllowResize) then
       result:=teePrev_None;
  end;
End;
    ...
    begin
      ...
      Case IDragged of
        ...
        teePrev_Image  : Begin
                           ...
                           if PointInRect(PaperRect,tmpR.TopLeft) and
                              PointInRect(PaperRect,tmpR.BottomRight) then
                                ImageRect:=tmpR;
                         End;
        { corners }
...
begin
  if Assigned(FPanels) and (Panels.Count>0) then result:=Panels[0]
                                            else result:=nil;
end;
