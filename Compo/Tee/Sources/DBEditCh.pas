...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        {$IFDEF CLX}
        Name:=TeeGetUniqueName(Self,'ComboFlat'); // Do not localize
        {$ENDIF}
      ...
      begin
        {$IFDEF CLX}
        Name:=TeeGetUniqueName(Self,'Label'); // Do not localize
        {$ENDIF}

        Alignment:=taRightJustify;
        Parent:=GroupFields;
        Top:=CBValues[t].Top+4;
        Caption:=tmpName+':';  // Do not localize
        Visible:=tmpName<>'';
      ...
      begin
        {$IFDEF CLX}
        Name:=TeeGetUniqueName(Self,'CheckBox'); // Do not localize
        {$ENDIF}
        ...
        Width:=Canvas.TextWidth(Caption + 'www'); // Do not localize
        Tag:=t;
        Visible:=tmpName<>'';
...
begin
  result:=(ASeries.DataSource is TDataSet) and
          (Copy(ASeries.MandatoryValueList.ValueSource,1,1)<>'#'); // Do not localize
end;

procedure TDBChartEditor.Translate;
var tmpArray : Array of TControl;
    t        : Integer;
begin
  if Assigned(TheSeries) then
     SetLength(tmpArray, TheSeries.ValuesList.Count+1)
  else
     tmpArray:=nil;
  ...
  try
    if Assigned(TheSeries) then
    begin
      tmpArray[0]:=CBLabelsField;

      for t:=0 to TheSeries.ValuesList.Count-1 do
          tmpArray[t+1]:=CBValues[t];
    end;

    {$IFNDEF D4}
    TeeTranslateControl(Self, tmpArray);
    {$ENDIF}
