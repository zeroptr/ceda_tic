...
begin
  ...
  LZoom.Caption:=IntToStr(SBZoom.Position)+'%'; // Do not localize
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Abs(HorizOffset)>SBHOffset.Max then  // 7.02
      begin
        SBHOffset.Max:=Round(Abs(HorizOffset)*1.5);
        SBHOffset.Min:=-SBHOffset.Max;
      end;

      SBHOffset.Position   :=HorizOffset;

      if Abs(VertOffset)>SBVOffset.Max then  // 7.02
      begin
        SBVOffset.Max:=Round(Abs(VertOffset)*1.5);
        SBVOffset.Min:=-SBVOffset.Max;
      end;
...
begin
  ...
  begin
    ...
    with AddNewChart(TeeMsg_ViewOrthoRight) do
    begin
      View3DOptions.Orthogonal:=True;
      Axes.Left.Labels:=True;
    end;

    with AddNewChart(TeeMsg_ViewOrthoLeft) do
    begin
      View3DOptions.Orthogonal:=True;
      Walls.Left.Hide;
    ...
    with AddNewChart(TeeMsg_View3DFront,True) do
    begin
    ...
    with AddNewChart(TeeMsg_View3DRight,True) do
    begin
      Walls.Left.Visible:=True;
      Walls.Right.Hide;
    ...
    with AddNewChart(TeeMsg_View3DLeft,True) do
    begin
      Walls.Left.Hide;
      Walls.Right.Visible:=True;
      Axes.Right.ZPosition:=0;
