...
type
  ...
  private
    ...
    function VerifyParent(var AControl:TWinControl):TWinControl;
  protected
  ...
  published
    ...
    property FontSeriesColor:Boolean read FFontSeriesColor
                                     write SetFontSeriesColor default False;
...
var
  TeeAddGalleryProc:Function(AOwner:TComponent; Chart:TCustomChart; Series:TChartSeries):TChartSeries=nil;
  TeeChangeGalleryProc:Function(AOwner:TComponent; var Series: TChartSeries):TChartSeriesClass=nil;

implementation

{$IFDEF CLR}
{$R 'TAreaSeries.bmp'} // Do not localize
{$R 'TArrowSeries.bmp'} // Do not localize
{$R 'TBarSeries.bmp'} // Do not localize
{$R 'TBubbleSeries.bmp'} // Do not localize
{$R 'TChartShape.bmp'} // Do not localize
{$R 'TFastLineSeries.bmp'} // Do not localize
{$R 'TGanttSeries.bmp'} // Do not localize
{$R 'THorizBarSeries.bmp'} // Do not localize
{$R 'TLineSeries.bmp'} // Do not localize
{$R 'TPieSeries.bmp'} // Do not localize
{$R 'TPointSeries.bmp'} // Do not localize
{$R 'THorizLineSeries.bmp'} // Do not localize
{$R 'THorizAreaSeries.bmp'} // Do not localize
{$ELSE}
{$R TeeBmps.res}
{$ENDIF}
...
{$IFNDEF CLX}
{$IFNDEF LCL}
// This (empty) method is necessary to force repaint after changing the listbox
// Color property. (VCL bug?)
procedure TChartListBox.CMColorChanged(var Message: TMessage);
begin
  inherited;
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      // Avoid VCL bug (Handle cannot be obtained)
      if Assigned(Parent) and Parent.Showing then
      begin
        tmpNum:=Items.Count-1;

        // Ensure only one series is active (visible)
        for t:=0 to tmpNum do
...
begin
  ...
  begin
    ...
    begin
      // Workaround a VCL bug when FOtherItems is a ComboBox.Items,
      // and the ComboBox.Parent(s) arent "Showing"

      try
        FOtherItems.Clear;
      except
        on E:EInvalidOperation do ;  // "eat" this exception
      end;
    end;
  end;
end;
...
{$ENDIF}
  ...
  function ChartShadow(AChart:TCustomAxisPanel):TTeeShadow;
  begin
    if Assigned(AChart) then
       result:=TCustomChart(AChart).Legend.Symbol.Shadow
    else
       result:=nil;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        tmpShadow:=ChartShadow(tmpSeries.ParentChart);

        if not Assigned(tmpShadow) then
           tmpShadow:=ChartShadow(ReferenceChart);

        if Assigned(tmpShadow) then
        with tmpShadow do
        begin
          OldShadowX:=HorizSize;
          OldShadowY:=VertSize;
      ...
      begin
        tmp:=SectionLeft(1);
        TeeDrawCheckBox(tmp+2,Rect.Top+((Rect.Bottom-Rect.Top-13) div 2),
                        tmpCanvas,tmpSeries.Active,Self.Color,CheckStyle=cbsCheck);
      end;
      ...
      begin
        if tmpSelected then
           Font.Color:={$IFNDEF CLX}ColorToRGB{$ENDIF}(clHighlightText)
        else
        if FFontSeriesColor then
           Font.Color:=tmpSeries.Color
        else
           Font.Color:=ColorToRGB(Self.Font.Color);
        ...
        TextOut(SectionLeft(3)+1,Rect.Top+((ItemHeight-TextHeight(TeeCharForHeight)) div 2),Items[Index]);
...
procedure TChartListBox.SetFontSeriesColor(const Value: Boolean);
begin
  if FFontSeriesColor<>Value then
  begin
    FFontSeriesColor:=Value;
    Invalidate;
  end;
end;
...
procedure TChartListBox.CheckLegendBoxes;
begin
  if Chart.Legend.CheckBoxes then
     CheckStyle:=Chart.Legend.CheckBoxesStyle;
end;
...
Begin
  tmpPanel:=VerifyParent(OldParent);
  ...
  begin
    // Workaround a VCL bug, when FOtherItems is a ComboBox.Items,
    // and the ComboBox.Parent(s) aren't "Showing"

    try
       FOtherItems.Assign(Items);
    except
       on E:EInvalidOperation do ; // "eat" this exception
    end;
  end;
...
begin
  // Workaround to VCL limitation to avoid exception
...
function TChartListBox.VerifyParent(var AControl:TWinControl):TWinControl;
begin
  AControl:=Parent;
  result:=nil;

  // This exception happens when a TabSheet has no parent.
  // Workaround is to use a temporary Form as Parent.
  try
    HandleNeeded;
  except
    on EInvalidOperation:Exception do
    begin
      try
...
procedure TChartListBox.TeeEvent(Event:TTeeEvent);
  ...
  begin
    ...
    tmp:=Items.IndexOfObject(TTeeSeriesEvent(Event).Series);
    ...
    begin
      ...
      if Assigned(FOtherItems) then
         FOtherItems.Delete(tmp);

      if Assigned(FOnRemovedSeries) then
         FOnRemovedSeries(Self,TTeeSeriesEvent(Event).Series);
    end;
...
begin
  ...
  Case Event of
    seChangeColor : Invalidate;
   seChangeActive : begin
                      CheckLegendBoxes;
                      Invalidate;
                    end;
    seChangeTitle : begin
...
begin
  ...
  // VCL Bug / Limitation:
  // If "not Parent.Showing" then Items.Count raises a
