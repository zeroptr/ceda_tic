...
uses {$IFNDEF LINUX}
     ...
     TeeNavigator, TeCanvas, TeeSourceEdit;
...
type
  ...
  TChartEditForm = class(TForm)
    ...
    N2: TMenuItem;
    extfont1: TMenuItem;
    SBCopyData: TSpeedButton;
    PopupSeries: TPopupMenu;
    ViewSeriesNames1: TMenuItem;
    ViewSeriesGroups1: TMenuItem;
    SpeedSeries: TSpeedButton;
    ...
    procedure SBCopyDataClick(Sender: TObject);
    procedure ViewSeriesNames1Click(Sender: TObject);
    procedure SpeedSeriesMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure ViewSeriesGroups1Click(Sender: TObject);
    procedure LBSeriesRemovedSeries(Sender: TChartListBox;
      Series: TCustomChartSeries);
  private
    ...
    SourceEditor   : TBaseSourceEditor;
    TreePanel      : TPanel;
    ...
    procedure FormSeriesEvent(Sender:TFormTeeSeries; Event:TTeeEvent);
...
const
  TeeMsg_RememberPosition='RememberPosition'; // Do not translate
  TeeMsg_RememberSize='RememberSize'; // Do not translate
  TeeMsg_TreeMode='TreeMode'; // Do not translate
  TeeMsg_TreeWidth='TreeWidth'; // Do not translate
  TeeMsg_DefaultTheme='DefaultTheme'; // Do not translate
  TeeMsg_DefaultPalette='DefaultPalette'; // Do not translate
  TeeMsg_SeriesGroups='SeriesGroups'; // Do not translate
...
uses {$IFNDEF LINUX}
     ...
     // ERROR: Including this unit at design-time on a package works fine under
     // Delphi 2005 but fails under BDS2006 Update 1 and leaves the help system
     // totally unstable under the IDE at design-time.
     //{$IFDEF D9}
     //  -->WinHelpViewer<--
     //{$ENDIF}

     TeeStore, TeeGalleryPanel, Math, TeeAbout;
...
begin
  ...
  if Old>Groups.Items.Count-1 then
     Old:=Groups.Items.Count-1;
...
begin
  ...
  SpeedSeries.Visible:=(csDesigning in Chart.ComponentState) or
                       TeeSeriesNamesRuntime or
                       (ceGroups in EditorOptions);

  ViewSeriesGroups1.Checked:=ceGroups in EditorOptions;
  ...
  if Assigned(Chart) and Tree.Visible then
     PrepareTree;
  ...
  if (Tree.Visible) and (not Assigned(TabSeriesList.Parent)) then
     TabSeriesList.Parent:=TreePanel;  // 7.07
  ...
  // Prepare grid to allow inserting new series by simply typing text or
  // pressing the right-arrow cursor key
  ChartGrid1.AllowInsertSeries:=True;
  ChartGrid1.ShowColors:=True;
end;

procedure TChartEditForm.ShowHideGroups;
begin
  // Series Groups
  ...
  PanelGroups.Visible:=ceGroups in EditorOptions;
  Splitter1.Visible:=PanelGroups.Visible;

  if PanelGroups.Visible then
     FillGroups;

//  if not BAddGroup.Visible then
     SpeedSeries.Left:=LBSeries.Left;
end;
...
procedure TChartEditForm.PrepareTree;
  ...
  function ChartTabsVisible:Boolean;
  var t : Integer;
  begin
    result:=True;

    for t:=0 to SubPage.PageCount-1 do
        if SubPage.Pages[t].TabVisible and (SubPage.Pages[t]<>TabSeriesList) then
           Exit;
...
begin
  ...
  begin
    ...
    if TabSeriesList.Visible or TabSeries.Visible then
    begin
    ...
    begin
      ...
      if SubPage.Pages[t]=TabWalls then
      begin
        AddChildObject(tmpNode2,'Left',Chart.Walls.Left);
        AddChildObject(tmpNode2,'Right',Chart.Walls.Right);
        AddChildObject(tmpNode2,'Bottom',Chart.Walls.Bottom);
        AddChildObject(tmpNode2,'Back',Chart.Walls.Back);
      end;
    end;
  ...
  {$IFNDEF TEEOCX} // Fix AX has no Parent Window
  Tree.SetFocus;
  {$ENDIF}
end;
...
begin
  ...
  Tree.Visible:=TeeReadBoolOption(TeeMsg_TreeMode,True);
  Tree.Width:=TeeReadIntegerOption(TeeMsg_TreeWidth,Tree.Width);
...
begin
  ...
  if Assigned(Chart) and Assigned(SubPage.ActivePage) then
  ...
  if SubPage.ActivePage=TabAxis then
     TFormTeeAxis(TabAxis.Controls[0]).PageAxisChange(Self);
end;
...
begin
  TabSeries.Parent:=MainPage; { 8.0 - TV52012047 }
...
begin
  ...
  if (Parent=nil) and LBSeries.CanFocus then
     ActiveControl:=LBSeries; { 5.01 }
end;
...
procedure TChartEditForm.FormSeriesEvent(Sender:TFormTeeSeries; Event:TTeeEvent);
begin
  if Event is TTeeSeriesEvent then
     with TTeeSeriesEvent(Event) do
          if (Event=seAdd) or (Event=seRemove) or (Event=seSwap) then
             AddSeriesNodes;
end;
...
begin
  ...
  tmp.Tools:=Chart.Tools;
  tmp.Align:=alClient;
  AddFormTo(tmp,TabTools);
...
procedure TChartEditForm.MainPageChange(Sender: TObject);
  ...
  begin
    ...
    begin
      ...
      FPreview.TeePreviewPanel1.Invalidate;
  end;
...
begin
  ...
  if Assigned(SourceEditor) then
     SourceEditor.FormCloseQuery(Sender,CanClose);
...
begin
  ...
  LBSeries.AllowAddSeries:=BAddSeries.Enabled;
  ...
  LBSeries.AllowDeleteSeries:=BDeleteSeries.Enabled;

  LBSeries.AskDelete:=(ceAskDelete in EditorOptions);
...
procedure TChartEditForm.LBSeriesRemovedSeries(Sender: TChartListBox;
  Series: TCustomChartSeries);
begin
  AddSeriesNodes;
end;
