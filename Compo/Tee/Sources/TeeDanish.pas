...
// TeeProCo
  ...
  TeeMsg_FunctionSubset   :='Delmængde';
  ...
  TeeMsg_Rotate           :='Roter';
  TeeMsg_Back             :='Tilbage';
  TeeMsg_Autosize         :='Autostørrelse';
  TeeMsg_Mirror           :='Spejl';
  TeeMsg_Columns          :='Kolonner';
  TeeMsg_Rows             :='Rækker';
  TeeMsg_Tile             :='Teglsten';
  TeeMsg_Bevel            :='Skråkant';
  TeeMsg_Zoom             :='Zoom';

  TeeMsg_Stop                 :='Stop';
  TeeMsg_Execute              :='Udfør !';
  TeeMsg_Themes               :='Tema';
  TeeMsg_LightTool            :='2D Lyssætning';
  ...
  TeeMsg_SurfaceNearestTool   :='Nærmest overfladen';
  ...
  TeeMsg_SeriesBandTool       :='Serier Bånd';

  TeeMsg_ThemeFacts           :='Fakta';
  TeeMsg_FunctionDownSampling :='Nedskalering';

  TeeMsg_SelectorTool         :='Udvælger';
  TeeMsg_LegendScrollBar      :='Tekst Scrollbar';

  TeeMsg_DataTableTool        :='Datatabel';

  TeeMsg_IsoSurfaceSeries     :='Iso-overflade';
  TeeMsg_UpToValue            :='Op til værdi';
  TeeMsg_NoCompression        :='Ingen kompression'; // TeeVideo tool

  TeeMsg_GalleryDarvas        :='Darvas';
  TeeMsg_VideoTool            :='Video kreatør';
  TeeMsg_VideoPlayerTool      :='Videoafspiller';

  TeeMsg_CursorToolDesc       :='Viser en trækbar markør-linie ovenpå serier.';
  TeeMsg_DragMarksDesc        :='Flytter seriemærker, når de trækkes med musen.';
  TeeMsg_DragPointDesc        :='Flytter et seriepunkt når det trækkes med musen.';
  TeeMsg_DrawLineDesc         :='Tillad tegning af brugervalgte linier ved træk med musen.';
  TeeMsg_ExtraLegendDesc      :='Viser en brugervalgt tekst et vilkårligt sted inden i diagrammet.';
  TeeMsg_GanttToolDesc        :='Tillad træk og omformning af Ganttbar.';
  TeeMsg_MarksTipDesc         :='Viser "tips" eller "hints" når en slut-bruger flytter eller trykker på muser over et seriepunkt.';
  TeeMsg_NearestToolDesc      :='Viser et grafisk signal når musen bliver flyttet nært et seriepunkt.';
  TeeMsg_PieToolDesc          :='Omridser eller udvider cirkeludsnit når der flyttes eller trykkes med musen.';
  TeeMsg_SeriesAnimationDesc  :='Udfører en voksende animation af seriepunkter.';
  TeeMsg_AxisArrowDesc        :='Viser trykbare pile ved akse start- og slut-punkter, for at kunne scrolle aksen.';
  TeeMsg_ColorBandDesc        :='Vise et farvelagt rektangeler (bånd) ved den angivne akse og position.';
  TeeMsg_ColorLineDesc        :='Viser en trækbar linie over aksen.';
  TeeMsg_GridBandDesc         :='Værktøj til gitterbånd, bruges det til at vise farvelagte rektangeler (bånd) ved gitterlinierne til den angivne akse og position.';
  TeeMsg_GridTransposeDesc    :='Bytter X og Z koordinaterne for at rotere Serierne 90 grader.';
  TeeMsg_AnnotationDesc       :='Viser en brugervalgt tekst på et vilkårligt sted i diagrammet.';
  TeeMsg_RectangleToolDesc    :='Viser en brugervalgt tekst på et vilkårligt sted i diagrammet inden i en omformbar boks.';
  TeeMsg_RotateDesc           :='Tillad roterende diagram ved træk med musen.';


  TeeMsg_ImageToolDesc        :='Viser et billede ved brug af de angivne serieakser som grænse. Når akserne bliver zommet eller scrollet, vil billedet følge de nye grænser.';
  TeeMsg_LightToolDesc        :='Viser en 2 dimensionel "lys" effekt over diagramarealet.';
  TeeMsg_SurfaceNearestDesc   :='Viser en visuel indikation ved udvælgelse af en overfladecelle, når brugeren flytter musen over en overfladeserie.';
  TeeMsg_AxisScrollDesc       :='Tillad scrolling af Akser ved træk af musen';
  TeeMsg_LegendScrollBarDesc  :='Tillad scrolling af diagramteksten.';
  TeeMsg_ClipSeriesDesc       :='Begrænser maling af serier udenfor serierassocierede aksegrænser.';
  TeeMsg_SelectorDesc         :='Slår udvælgelse og træk af diagram elementer og annotationer til.';
  ...
  TeeMsg_TransposeSeriesDesc  :='Skifter Serier rækker og kolonner.';
  TeeMsg_GridTranspDesc       :='Skifter 3D Serier XYZ celledata (X på Z)';
  TeeMsg_LegendScrollDesc     :='Viser en scrollbar ved diagrammets tekst for at muliggør scroll ved at trække med musen.';
  TeeMsg_DataTableDesc        :='Viser et gitter med Seriernes dataværdier.';
  TeeMsg_SeriesBandDesc       :='Fylder området mellem to serier.';

  TeeMsg_ThemeRandom          :='Tilfældig';
  TeeMsg_Play                 :='Afs&pil';
  TeeMsg_GalleryHighLowLine   :='HøjLav Linie';
  TeeMsg_Mirrored             :='Spejlede';
  TeeMsg_PolarGridSeries      :='Polar Gitter';

  TeeMsg_FullScreen           :='Fuld skærm';
  TeeMsg_FullScreenDesc       :='Vis diagram maksimerede til fuld skærm';

  TeeMsg_MagnifyTool          :='Forstørrelsesglas';
  TeeMsg_MagnifyToolDesc      :='Forstørre en del af diagrammet';
...
'BLINK DELAY ON=Forsink blink slået til'#13+
'BLINK DELAY OFF=Forsink blink slået fra'#13+
'SCROLL DIRECTION=ScrollRetning'#13+
'BLINK=Blink'#13+
'RIGHT TO LEFT=Højre til venstre'#13+
'LEFT TO RIGHT=Venstre til højre'#13+
'SLANT CUBE=Kube drejning'#13+
'INVERT. ARROW=Invert. pil'#13+
'INVERT. CONE=Invert. kegle'#13+
'TICK LINES=Liniemærker'#13+
...
'RELATIVE TO OPEN=Relativ til åben'#13+
'RELATIVE TO PREVIOUS CLOSE=Relativ til forgående luk'#13+
'VIEW=Vis'#13+
'EDIT COLOR=Editer farver'#13+
'MAKE NULL POINT=Lav NULL punkter'#13+
'APPEND ROW=Tilføj række'#13+
'TEXT FONT=Skrifttype'#13+
'VIEW SERIES NAMES=Vis serienavn'#13+
'VIEW SERIES GROUPS=Vis seriegruppe'#13+
'CHART THEME SELECTOR=Diagram temaudvælger'#13+
'THEMES=Tema'#13+
...
'SELECT NEW LINES=Vælg nye linier'#13+
'HORIZ. PARALLEL=Horis. parallel'#13+
'VERT. PARALLEL=Vert. parallel'#13+
'EXCEL FILE=Excel fil'#13+
'WORKSHEET=Ark'#13+
'VALUES RANGE=Rækkevide for værdier'#13+
'FROM=Fra'#13+
'LABELS RANGE=Rækkevide for labels'#13+
...
'ZOOM ON UP LEFT DRAG=Zoom slået til ved venstre træk'#13+
'SPOT=Punkt'#13+
'FIXED POSITION=Fast position'#13+
'SET DIRECTION=Sæt retning'#13+
...
'INVERTED GRAY=Inverterede grå'#13+
'RAINBOW=Regnbue'#13+
...
'BANDS PEN=båndpen'#13+
'TEECHART LIGHTING=Teechart lyssætning'#13+
'LINEAR=Lineær'#13+
...
'MODE=Modus'#13+
'REMEMBER SIZE=Husk størrelse'#13+
...
'DISABLE=Slå fra'#13+
'COLOR EACH SLICE=Farvelæg hver del'#13+
'CALLOUT LEG SIZE=Talebobel størrelse'#13+
...
'THEME=Tema'#13+
'NæSTE >=Næste >'#13+
'TEXT QUOTES=Tekstanførelsestegn'#13+
'NAME=Navn'#13+
'ITALIC=Kursiv'#13+
'BOLD=Fed'#13+
...
'LUK LOKATION VæRDI=Luk lokationsværdi'#13+
