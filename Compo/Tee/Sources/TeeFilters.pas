...
var
  ...
  sum, cMax, cMin: Word;
  InvDif,
  Rdelta, Gdelta, Bdelta: Single; { intermediate value: % of spread from max }
begin
  R:=Color.Red;
  G:=Color.Green;
  B:=Color.Blue;
  ...
  L := ( (sum * HLSMAX) + RGBMAX ) * Inv2RGBMAX;

  if cMax = cMin then  { r=g=b --> achromatic case }
  begin                { saturation }
    Hue := HLSUndefined;  // pwHue := 160;      { MS ColoroHLS always defaults to 160 in this case }
    Luminance := Round(L);
    Saturation := 0;
  end
  else                 { chromatic case }
  begin
    ...
    Rdelta := ( ((cMax-R)*HLSMAXDiv6) + (dif*0.5) ) * InvDif;
    Gdelta := ( ((cMax-G)*HLSMAXDiv6) + (dif*0.5) ) * InvDif;
    Bdelta := ( ((cMax-B)*HLSMAXDiv6) + (dif*0.5) ) * InvDif;

    if R = cMax then
       H := Bdelta - Gdelta
    else
    if G = cMax then
       H := HLSMAXDiv3 + Rdelta - Bdelta
    else // B == cMax
...
procedure HLSToRGB(Hue, Luminance, Saturation: Word; out rgb: TRGB);
  ...
  function HueToRGB(const Lum, Sat, Hue:Float): Integer;
  const
    InvHLSMAXDiv6=1/HLSMAXDiv6;
  begin
    { return r,g, or b value from this tridrant }
    if hue < HLSMAXDiv6 then
        Result := Round( Lum + (((Sat-Lum)*hue+HLSMAXDiv12)*InvHLSMAXDiv6))
    else
    if hue < HLSMAXDiv2 then
        Result := Round( Sat)
    else
    if hue < HLSMAX2Div3 then
        Result := Round( Lum + (((Sat-Lum)*(HLSMAX2Div3-hue)+HLSMAXDiv12)*InvHLSMAXDiv6) )
    else
        Result := Round( Lum );
  end;
...
var
  ...
  function RoundColor2(Hue: Float): Integer;
  begin
    ...
    result:=RoundColor(Round((HueToRGB(Magic1,Magic2,Hue)*RGBMAX + HLSMAXDiv2)*InvHLSMAX));
  end;
...
begin
  ...
  begin            { achromatic case }
    Red := RoundColor(Round((Luminance * RGBMAX)*InvHLSMAX) );
    Green:=Red;
    Blue:=Green;
  ...
  begin            { chromatic case }
    ...
    if Luminance <= HLSMAXDiv2 then
       Magic2 := (Luminance * (HLSMAX + Saturation) + HLSMAXDiv2) * InvHLSMAX
    else
       Magic2 := Luminance + Saturation - ((Luminance * Saturation) + HLSMAXDiv2) * InvHLSMAX;
    ...
    with rgb do
    begin            { achromatic case }
      Red:=RoundColor2(Hue+HLSMAXDiv3);
      Green:=RoundColor2(Hue);
      Blue:=RoundColor2(Hue-HLSMAXDiv3);
    end;
  end;
end;
