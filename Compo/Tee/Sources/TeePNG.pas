...
{$ENDIF}
  ...
  private
    ...
    PicData : TPicData;
    RowPtrs : PByte;
    SaveBuf : packed Array[0..8192] of Byte;
    {$ENDIF}
...
Const 
  TeePNG_DefaultCompressionLevel=9;

{$IFDEF TEEOCX}
var 
  PNGSection : TCriticalSection;
{$ENDIF}
  ...
  begin
    for y:=0 to PicData.Height-1 do
      ...
      begin
        ...
        PByte(AktP+0)^:=BYTE((AktPicP+2)^);
        PByte(AktP+1)^:=BYTE((AktPicP+1)^);
        PByte(AktP+2)^:=BYTE((AktPicP+0)^);
      end;
  end;
...
{$ELSE}
{$IFNDEF LINUX}
var tmp : {$IFDEF D12}TPNGImage{$ELSE}TPNGObject{$ENDIF};
    tmpBitmap : TBitmap;
begin
  CheckSize;

  tmp:={$IFDEF D12}TPNGImage{$ELSE}TPNGObject{$ENDIF}.Create;
  try
    tmpBitmap:=Bitmap;
    tmp.Assign(tmpBitmap);

    tmp.CompressionLevel:=CompressionLevel;
