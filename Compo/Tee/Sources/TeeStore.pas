...
Procedure SaveChartToXMLFile(AChart:TCustomChart; Const AFileName:String;
                             IncludeData:Boolean=True; XMLHeader:Boolean=True);
...
Procedure LoadChartFromStreamCheck( Var AChart:TCustomChart;
                                    ...
                                    // If you need compatibility with version 5 saved
                                    // charts, pass TryReadData=TRUE.
                                    TryReadData:Boolean=False  // 7.07
                               );
...
type
  ...
  public
    ...
    Function ValueToString(ASeries:TChartSeries; const Value:Double):String;
  ...
  published
    ...
    // property IncludeSeriesTitle:Boolean read FIncludeSeriesTitle write FIncludeSeriesTitle default False;

    property UseSeriesFormat:Boolean read FUseSeriesFormat write FUseSeriesFormat default False;
...
begin
  ...
  if not result then
     result:={$IFDEF USEANSI}AnsiChar{$ELSE}Char{$ENDIF}(Header.Magic) in ['o','O'];
...
Function TSeriesData.ValueToString(ASeries:TChartSeries; const Value:Double):String;
var tmp : String;
begin
  if Assigned(ASeries) and UseSeriesFormat then
     tmp:=ASeries.ValueFormat
  else
     tmp:=ValueFormat;
    ...
    begin
      if ASeries.Count>Index then
         if ValueList.DateTime then
            ...
            result:=ValueToString(ASeries,ValueList.Value[Index])
      else
         result:='';
    end;
...
Function TSeriesDataXML.AsString:String;
  ...
  Function SeriesPoints(ASeries:TChartSeries):String;
    ...
    Function GetPointString(Index:Integer):String;
      ...
      begin
        if  Aseries.IsNull(Index) and ( AList.Name <> 'X') then  // TV52013076
           result := ''
        else
           result:=' '+AList.Name+'="'+ValueToString(ASeries,AList.Value[Index])+'"';
      end;
    ...
    begin
      ...
      begin
        ...
        if tfLabel in IFormat then
           result:=result+' text="'+Labels[Index]+'"';
        ...
        { the "X" point value, if exists }
        if tfNoMandatory in IFormat then
           AddResult(Get(NotMandatoryValueList));

        if not IsNull(Index) then // TV52013076
        begin
          { the "Y" point value }
          ...
          for tt:=2 to ValuesList.Count-1 do
              result:=result+Get(ValuesList[tt]);
        end;
      end;
    end;
...
function TSeriesDataHTML.AsString: String;
  ...
  Function Header:String;
    ...
    begin
      ...
      if tfLabel in IFormat then
         if (not FirstSeriesLabels) or
            (ASeries.ParentChart.SeriesList.IndexOf(ASeries)=0) then
             AddCol(TeeMsg_Text);

      if tfColor in IFormat then
         AddCol(TeeMsg_Colors);

      with ASeries do
      begin
        if tfNoMandatory in IFormat then
           AddCol(NotMandatoryValueList.Name);

        if ValuesList.Count=2 then
           AddCol(SeriesTitleOrName(ASeries))
        else
        begin
          AddCol(MandatoryValueList.Name);

          for t:=2 to ValuesList.Count-1 do
              AddCol(ValuesList[t].Name);
        end;
      end;
    end;
  ...
  begin
    result:='<tr>';
    if IncludeIndex then
       result:=result+'<td>'+TeeMsg_Index+'</td>';

    if Assigned(FSeries) then
       result:=result+HeaderSeries(FSeries)
    else
    if Assigned(FChart) then
    for t:=0 to FChart.SeriesCount-1 do
        result:=result+HeaderSeries(FChart[t]);
...
begin
  ...
  if IncludeHeader then
     result:=result+Header+TeeTextLineSeparator;
...
Function TSeriesDataHTML.PointToString(Index:Integer):String;
  ...
  Function GetPointString:String;
    ...
    Function GetPointStringSeries(ASeries:TChartSeries):String;
      ...
      begin
        result:='<td>'+ValueToString(ASeries,Value)+'</td>';
      end;
    ...
    begin
      ...
      begin
        ...
        if tfLabel in IFormat then
           if (not FirstSeriesLabels) or
              (ASeries.ParentChart.SeriesList.IndexOf(ASeries)=0) then
           result:=result+'<td>'+Labels[Index]+'</td>';
  ...
  begin
    ...
    if Assigned(FSeries) then
       result:=result+GetPointStringSeries(FSeries)
    else
    for t:=0 to FChart.SeriesCount-1 do
...
var Buf : Array[0..4] of Word;
  ...
  Procedure WriteSeries(ASeries:TChartSeries);
    ...
    procedure WriteValue(const AList:TChartValueList);
    var tmp : String;
    begin
      if UseSeriesFormat then
         tmp:=ASeries.ValueFormat
      else
         tmp:=ValueFormat;

      if tmp='' then
         WriteDouble(AList.Value[Row])
      else
         // Trick, use format string to "round" value:
         WriteDouble(StrToFloat(ValueToString(ASeries,AList.Value[Row])));
    end;
  ...
  begin
    ...
    begin
      ...
      if tfNoMandatory in IFormat then
      begin
        WriteValue(ASeries.NotMandatoryValueList);
        Inc(Col);
      end;
      ...
      begin
        WriteValue(ASeries.MandatoryValueList);

        Inc(Col);

        for tt:=2 to ASeries.ValuesList.Count-1 do
        begin
          WriteValue(ASeries.ValuesList[tt]);
          Inc(Col);
        end;
      end;
    end;
