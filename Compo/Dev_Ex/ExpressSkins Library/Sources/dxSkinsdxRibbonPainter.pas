...
type
  ...
  public
    ...
    // Contexts
    procedure DrawContextBackground(DC: HDC; const R: TRect); override;
    procedure DrawContextBackgroundGlass(DC: HDC; const R: TRect); override;
...
procedure TdxSkinRibbonPainter.DrawApplicationMenuContentHeader(
...
begin
  ...
  if AElement = nil then
    inherited DrawContextBackground(DC, R)
  else
  begin
    ARect := R;
    Inc(ARect.Bottom);
...
procedure TdxSkinRibbonPainter.DrawContextBackgroundGlass(DC: HDC; const R: TRect);
var
  AElement: TdxSkinElement;
  ASkinInfo: TdxSkinLookAndFeelPainterInfo;
begin
  ...
  if AElement = nil then
    inherited DrawContextBackgroundGlass(DC, R)
  else
    AElement.Draw(DC, R);
end;
...
begin
  ...
  if AElement = nil then
    ...
    AElement.Draw(DC, R, 0,
      SkinElementCheckState(AElement, SkinElementStateByRibbonState(AState)));
end;
...
procedure TdxSkinRibbonPainter.DrawMarkArrow(
  DC: HDC; const R: TRect; AState: Integer);
var
  AElement: TdxSkinElement;
  ASkinInfo: TdxSkinLookAndFeelPainterInfo;
...
const
  StateMap: array[0..8] of TdxSkinElementState = (
    esNormal, esNormal, esHot, esActive, esPressed,
    esFocused, esActive, esHot, esNormal
  );
var
  AElement: TdxSkinElement;
  ASkinInfo: TdxSkinLookAndFeelPainterInfo;
...
begin
  ...
  begin
    case APart of
      DXBAR_KEYTIP_TEXTCOLOR, DXBAR_SCREENTIP_TITLE:
...
var
  ...
  ASkinInfo: TdxSkinLookAndFeelPainterInfo;
begin
  if APart = DXBAR_APPLICATIONBUTTONICONOFFSET then
  begin
    Result := Rect(9, 9, 9, 9);
    Exit;
  end;
...
begin
  ...
  begin
    case APart of
      DXBAR_TOOLBAR:
        Result := ASkinInfo.RibbonIndents[3];
      DXBAR_TABSGROUPSOVERLAPHEIGHT:
        Result := ASkinInfo.RibbonIndents[2];
      else
