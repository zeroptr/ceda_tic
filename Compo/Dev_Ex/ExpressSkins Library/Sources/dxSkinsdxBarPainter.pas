...
procedure TdxBarSkinPainter.InternalDrawDockedBarBackground(
  ...
  procedure DrawMainMenuDragVerticalElement(ABarControlAccess: TdxBarControlAccess;
    DC: HDC; const R: TRect);
  var
    ABitmap: TcxAlphaBitmap;
  begin
    ABitmap := TcxAlphaBitmap.CreateSize(cxRectRotate(R));
    try
  ...
  var
    AElement: TdxSkinElement;
  begin
    if ABarControlAccess.IsMainMenu and ABarControlAccess.Vertical then
      DrawMainMenuDragVerticalElement(ABarControlAccess, ACanvas.Handle, R)
    else
    begin
      if ABarControlAccess.IsMainMenu then
