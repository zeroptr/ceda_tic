...
type
  ...
  { TdxSkinLookAndFeelPainterCacheManager }

  TdxSkinLookAndFeelPainterCacheManager = class(TObject)
  private
    FPrintingPreviewBackground: TdxSkinElementCache;
    FScrollBarBackground: array [Boolean] of TdxSkinElementCache;
  public
    constructor Create; virtual;
    destructor Destroy; override;
    procedure DrawScrollBarBackground(DC: HDC; const R: TRect;
      AElement: TdxSkinElement; AHorizontal: Boolean);
    procedure DrawPreviewBackground(DC: HDC; const R: TRect; AElement: TdxSkinElement);
  end;

var
  CacheManager: TdxSkinLookAndFeelPainterCacheManager;
...
begin
  ...
  if (AScrollInfoElement = nil) or (AScrollInfoElement.Element = nil) then
    ...
    CacheManager.DrawScrollBarBackground(
      ACanvas.Handle, R, AScrollInfoElement.Element, AHorizontal);
end;
...
const
  ...
  begin
    with CacheData do
      if EditButtonElements[False] = nil then
        ...
        AColor := EditButtonElements[False].TextColor;
  ...
  function GetContentRect(const R: TRect; AElement: TdxSkinElement): TRect;
  begin
...
begin
  ...
  begin
    R := GetContentRect(ARect, CacheData.EditButtonElements[False]);
    case AButtonKind of
      cxbkEllipsisBtn:
        begin
          ASize := EllipseSizeMap[cxRectWidth(R) >= 12];
          DrawEllipsis(cxRectCenter(R, 3 * ASize + 4, ASize), ASize);
        end;

      cxbkComboBtn, cxbkEditorBtn, cxbkSpinUpBtn, cxbkSpinDownBtn,
      cxbkSpinLeftBtn, cxbkSpinRightBtn:
        begin
          AGlyph := CacheData.EditButtonGlyphs[AButtonKind];
          if AGlyph <> nil then
...
begin
  DrawEditorButtonBackground(ACanvas, R, False, ButtonStateToSkinState[AState]);
end;
...
begin
  with CacheData do
    if PrintingPreviewBackground =  nil then
      ...
      CacheManager.DrawPreviewBackground(ACanvas.Handle, R, PrintingPreviewBackground);
end;
...
constructor TdxSkinLookAndFeelPainterCacheManager.Create;
begin
  FScrollBarBackground[True] := TdxSkinElementCache.Create;
  FScrollBarBackground[False] := TdxSkinElementCache.Create;
  FPrintingPreviewBackground := TdxSkinElementCache.Create;
end;

destructor TdxSkinLookAndFeelPainterCacheManager.Destroy;
begin
  FreeAndNil(FPrintingPreviewBackground);
  FreeAndNil(FScrollBarBackground[False]);
  FreeAndNil(FScrollBarBackground[True]);
  inherited Destroy;
end;

procedure TdxSkinLookAndFeelPainterCacheManager.DrawScrollBarBackground(
  DC: HDC; const R: TRect; AElement: TdxSkinElement; AHorizontal: Boolean);
begin
  FScrollBarBackground[AHorizontal].DrawEx(DC, AElement, R);
end;

procedure TdxSkinLookAndFeelPainterCacheManager.DrawPreviewBackground(
  DC: HDC; const R: TRect; AElement: TdxSkinElement);
begin
  FPrintingPreviewBackground.DrawEx(DC, AElement, R);
end;
...
initialization
  CacheManager := TdxSkinLookAndFeelPainterCacheManager.Create;
  PaintersManager := GetExtendedStylePainters;

finalization
  FreeAndNil(CacheManager);
  PaintersManager := nil;
