...
{$IFDEF DELPHI6}
  Variants, DateUtils,
{$ENDIF}
  Classes, SysUtils, Windows, Forms, Math, Graphics, Contnrs, cxClasses, cxCustomData,
  cxDataStorage, cxDateUtils, cxDataUtils, cxSchedulerUtils, cxVariants, cxStorage,
  ExtCtrls, cxLookAndFeels, Controls, ImgList, cxFormats, cxSchedulerHolidays, cxDataConsts, dxCore;
...
begin
  ...
  begin
    ...
    Result.Start := ACalculator.OccurrenceStart - Result.TimeBias +
      GetTimeBiasDaylightSavingTime(ACalculator.OccurrenceStart) + Storage.TimeBias;
    Result.Finish := ACalculator.OccurrenceFinish - Result.TimeBias +
      GetTimeBiasDaylightSavingTime(ACalculator.OccurrenceStart) + Storage.TimeBias;
    Result.RecurrenceIndex := ACalculator.Index;
    Result.EventType := etOccurrence;
...
begin
  ...
  Result.FTimeBias := FTimeBias + GetTimeBiasDaylightSavingTime(Start);
end;
...
begin
  ...
  if DaylightSaving then
    Result := TcxSchedulerDateTimeHelper.TimeZoneDaylightBias(Start, FTimeZone) * MinuteToTime;
end;
