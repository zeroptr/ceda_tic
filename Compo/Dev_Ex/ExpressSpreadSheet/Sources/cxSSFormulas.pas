...
const
  ...
  begin
    ...
    AFracValue := Frac(AValue) * APrecision;
    Result := Int(AFracValue);
    AFracValue := Abs(Frac(AFracValue));
    if CompareValue(AFracValue, LimitValue) <> -1 then
      Result := Result + ValueIncr[Result >= 0];
    Result := Result / APrecision;
  end;
...
begin
  ...
  try
    ...
    AValue := Int(AValue) + xlRound(AValue, ADigits);
  finally
