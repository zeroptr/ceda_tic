...
begin
  Result := inherited GetBorderExtent;
  if not (IsInplace or (AViewInfo = nil) or (AViewInfo.Painter = nil)) then
  begin
    if IsPanelStyle then
  ...
  begin
    AHeaderSideClientExtent := cxTextHeight(Edit.FCaptionFont) + Result.Top + cxEditMaxBorderWidth + 1;
    case Edit.FAlignment of
      alTopLeft, alTopCenter, alTopRight:
...
begin
  ...
  begin
    ...
    if ViewInfo.Painter <> nil then
      ATextColor := ViewInfo.Painter.GroupBoxTextColor(Enabled, cxGroupBoxAlignment2GroupBoxCaption(Alignment))
    else
      if IsNativeStyle then
        ATextColor := TcxWinXPLookAndFeelPainter.GroupBoxTextColor(Enabled, cxGroupBoxAlignment2GroupBoxCaption(Alignment));
...
begin
  Result := RedrawOnResize and (IsNativeStyle or Assigned(LookAndFeel.SkinPainter));
end;
