...
type
  ...
  protected
    ...
    function IsLineDrawStyleNeeded: Boolean; virtual;
  public
...
uses
  cxRichEdit, dxOffice11, cxTextEdit, ComCtrls, RichEdit, ComObj,
      ...
      begin
        if Terminated then
          ...
          if CheckMode.InternalProcessWord(AWord) then
            Continue;
        if CheckMode.CheckWord(AWord) <> scNoError then
        begin
          if Terminated then
            Break
          else
...
begin
  ...
  try
    if not MisspelledWords.FindWord(AWord, True, AItem) then
      AItem := MisspelledWords.AddWord(FMisspellingStart, AWord);
    AMisspelled := TdxSpellCheckerMisspelledWord(AItem);
    AMisspelled.Checked := True;
    AMisspelled.Position := FMisspellingStart;
...
begin
  ...
  if not Adapter.IsMultiLine then
    CalculateSimpleBounds(AWord, AWord.Position - 1, Length(AWord.Word))
  else
  begin
    AStartLineIndex := Adapter.GetLineIndex(AWord.Position - 1);
    AFinishLineIndex := Adapter.GetLineIndex(AWord.Position + Length(AWord.Word) - 2);
...
procedure TdxSpellCheckerCheckAsYouTypeMode.SetMisspelledWordInfo(
  AWord: TdxSpellCheckerMisspelledWord);
begin
  FMisspellingStart := AWord.Position;
  FMisspellingLen := AWord.EndPosition - FMisspellingStart;
...
begin
  ...
  begin
    ...
    CorrectWords.AddWord(FMisspellingStart, AWord);
    Ignore;
  end;
end;
...
begin
  ...
  ReleaseDC(EditorHandle, DC);
  Result := ATextMetric.tmExternalLeading + ATextMetric.tmHeight;
end;

function TdxSpellCheckerCheckAsYouTypeEditAdapter.IsLineDrawStyleNeeded: Boolean;
begin
  Result := GetLineHeightByCharIndex(0) >= cxRectHeight(TextAreaRect);
end;
...
begin
  ...
  begin
    AWord := Items[I];
    Result := (AIndex >= AWord.Position) and (AIndex < AWord.EndPosition);   
    if Result then
      Break;
  end;
end;
