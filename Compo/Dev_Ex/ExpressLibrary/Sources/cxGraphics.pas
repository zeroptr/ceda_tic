...
procedure cxPaintControlTo(ADrawControl: TWinControl; ACanvas: TcxCanvas;
  const ACanvasOffset, AWindowOffset: TPoint; ADrawRect: TRect; ADrawEdgesAndBorders: Boolean; AConsiderWindowRegion: Boolean; ASkipControls: TList);
  ...
  procedure PaintChildTo(AChildControl: TControl);
  var
    P: TPoint;
  ...
  begin
    ...
    if (AChildControl is TWinControl) and (AChildControl as TWinControl).Visible then
      cxPaintControlTo((AChildControl as TWinControl), ACanvas, P, ABoundsRect.TopLeft, ADrawRect, True, AConsiderWindowRegion, ASkipControls);
  end;
...
begin
  ...
  begin
    ...
    try
      AList.Add(AControl);
      cxPaintControlTo(AControl.Parent, ACanvas, cxPointInvert(ARect.TopLeft), cxPointInvert(AControl.BoundsRect.TopLeft), ARect, False, True, AList);
    finally
      AList.Free;
    end;
  end;
...
procedure cxPaintTo(ADrawControl: TWinControl; ACanvas: TcxCanvas; const AOffset: TPoint; const ADrawRect: TRect; AConsiderWindowRegion: Boolean);
begin
  MoveWindowOrg(ACanvas.Handle, -ADrawRect.Left, -ADrawRect.Top);
  cxPaintControlTo(ADrawControl, ACanvas, AOffset, cxNullPoint, ADrawRect, True, AConsiderWindowRegion, nil);
end;
