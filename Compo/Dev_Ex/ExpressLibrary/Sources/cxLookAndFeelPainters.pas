...
var
  ...
  R: TRect;
begin
  if IsRectEmpty(ATextRect) or (SchedulerNavigationButtonTextColor(AIsNextButton, AState, clDefault) = clNone) then
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Font.Color := SchedulerNavigationButtonTextColor(AIsNextButton, AState, Font.Color);
...
begin
  ATheme := OpenTheme(totButton);
  if ATheme <> 0 then
    GetThemeColor(ATheme, BP_GROUPBOX, GroupBoxStateMap[AEnabled], TMT_TEXTCOLOR, DWORD(Result))
  else
    Result := inherited GroupBoxTextColor(AEnabled, ACaptionPosition);
end;
