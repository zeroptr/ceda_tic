...
type
  ...
  public
    function IsDoubleBufferNeeded(DC: HDC): Boolean;
...
begin
  ...
  ABitmap := TcxBitmap32.CreateSize(Texture.Width, Texture.Height, True);
  try
    ASize := Size;
...
function TdxSkinCanvas.IsDoubleBufferNeeded(DC: HDC): Boolean;
begin
  Result := (GetDeviceCaps(DC, BITSPIXEL) <= 8) or (GetDeviceCaps(DC, NUMCOLORS) > 1);
end;
...
begin
  ...
  ATempDC := DC;
  FSourceDC := DC;
  if IsDoubleBufferNeeded(DC) then
  begin
    FBuffer := TcxBitmap.CreateSize(R, pf32bit);
...
begin
  ...
  if FBuffer <> nil then
  begin
    FBuffer.cxCanvas.WindowOrg := cxNullPoint;
...
begin
  if (AColor1 = AColor2) or (AColor2 = clNone) or (AColor2 = clDefault) then
    ...
    begin
      ...
      dxGpFillRectByGradient(Graphics, R,
...
procedure TdxSkinCanvas.StretchDrawImage(AImage: TdxPNGImage; const ADest, ASource: TRect);
begin
...
var
  ...
  RDest, RSrc: TRect;
begin
  if dxGpIsRectVisible(Graphics, ADest) then
  begin
    AOldIntpMode := InterpolationMode;
    try
      InterpolationMode := InterpolationModeNearestNeighbor;
