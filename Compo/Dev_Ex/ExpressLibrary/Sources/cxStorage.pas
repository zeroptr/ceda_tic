...
type
  ...
  private
    FRegistry: TRegistry;
    function CorrectStringValue(const S: string): string;
  public
...
begin
  Result := (Length(AValue) >= 2) and (AValue[1] = '"') and (AValue[Length(AValue)] = '"');
end;
  ...
  case FRegistry.GetDataType(AName) of
    ...
    begin
      ...
      if IsStringValue(AValue) then
        Result := CorrectStringValue(AValue)
      else
      begin
...
function TcxRegistryReader.CorrectStringValue(const S: string): string;
begin
  Result := S;
  Delete(Result, 1, 1);
...
function TcxRegistryWriter.CorrectStringValue(const S: string): string;
begin
  Result := '"' + S + '"';
end;
...
begin
  FStorageVersion := FIniFile.ReadInteger('Main', 'Version', 0);
end;
...
function TcxIniFileReader.CorrectStringValue(const S: string): string;
begin
  Result := S;
  Delete(Result, 1, 1);
  Delete(Result, Length(Result), 1);
  if FStorageVersion = 1 then
  begin
...
//  AFileName := ChangeFileExt(AStorageName, '.ini');
  FIniFile := TMemIniFile.Create(AStorageName{$IFDEF DELPHI12}, TEncoding.Unicode{$ENDIF});
...
function TcxIniFileWriter.CorrectStringValue(const S: string): string;
begin
