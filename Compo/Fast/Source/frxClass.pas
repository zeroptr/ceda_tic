...
{$ENDIF}
  ...
  published
    ...
    property SwapPageSize: Boolean read FSwapPageSize write FSwapPageSize default False;
  end;
    ...
    begin
      if TfrxBand(c).Vertical then
        ...
        if (Columns > 1) and not((c is TfrxNullBand) or (c is TfrxReportSummary) or
          (c is TfrxPageHeader) or (c is TfrxPageFooter) or (c is TfrxReportTitle)) then
...
begin
  ...
  try
    ...
    Export2PDF.OwnerPassword := OwnerPassword;
    Export2PDF.UserPassword := UserPassword;
