...
const
  ...
  navigator =
    ...
    '<meta name=Generator content="FastReport 4.0 http://www.fast-report.com">'#13#10 +
  ...
  report = '';
  print =
    '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><meta name=Generator content="FastReport 4.0 http://www.fast-report.com">' +
...
begin
  FSSI.Clear;
  FTemplate.SaveToStream(FSSI{$IFDEF Delphi12}, TEncoding.ASCII{$ENDIF});
  FSSI.Prepare;
  FTemplate.Clear;
  FSSI.Position := 0;
...
begin
  ...
  if FileExists(path) then
    FTemplate.LoadFromFile(path{$IFDEF Delphi12}, TEncoding.ASCII {$ENDIF})
