object frmKulSifreDegis: TfrmKulSifreDegis
  Left = 446
  Top = 299
  ActiveControl = txtSIFRE
  BorderIcons = [biSystemMenu]
  BorderStyle = bsDialog
  Caption = 'Kullan'#305'c'#305' '#350'ifre De'#287'i'#351'tirme'
  ClientHeight = 217
  ClientWidth = 332
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 180
    Width = 332
    Height = 37
    Align = alBottom
    TabOrder = 0
    object btnCIKIS: TSpeedButton
      Left = 172
      Top = 8
      Width = 73
      Height = 25
      Caption = #304'pt&al'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        11B2050FB10107A0000188FF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentFont = False
      OnClick = btnIptalClick
    end
    object btnTAMAM: TBitBtn
      Left = 252
      Top = 8
      Width = 73
      Height = 25
      Caption = '&Tamam'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = btnTamamClick
      Glyph.Data = {
        
        9923139923139923138C2AFF00FFFF00FFFF00FFFF00FFFF00FF}
      Style = bsNew
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 332
    Height = 180
    Align = alClient
    TabOrder = 1
    object GroupBox1: TGroupBox
      Left = 1
      Top = 1
      Width = 330
      Height = 77
      Align = alTop
      Caption = 'Kullan'#305'c'#305' Bilgileri'
      Enabled = False
      TabOrder = 1
      object IB_Edit1: TIB_Edit
        Left = 90
        Top = 19
        Width = 235
        Height = 21
        AutoLabel.Kind = albLeft
        AutoLabel.Caption = 'Ad'#305' Soyad'#305
        AutoLabel.RelativePosition = -80
        DataField = 'AD_SOY'
        DataSource = dtsKullanici
        TabOrder = 0
      end
      object IB_Edit2: TIB_Edit
        Left = 90
        Top = 46
        Width = 235
        Height = 21
        AutoLabel.Kind = albLeft
        AutoLabel.Caption = 'Kullan'#305'c'#305' Ad'#305
        AutoLabel.RelativePosition = -80
        DataField = 'USER_NAME'
        DataSource = dtsKullanici
        TabOrder = 1
      end
    end
    object GroupBox2: TGroupBox
      Left = 1
      Top = 78
      Width = 330
      Height = 101
      Align = alClient
      Caption = #350'ifre Bilgileri'
      TabOrder = 0
      object Label1: TLabel
        Left = 11
        Top = 20
        Width = 21
        Height = 13
        Caption = #350'ifre'
      end
      object Label2: TLabel
        Left = 11
        Top = 47
        Width = 45
        Height = 13
        Caption = 'Yeni '#350'ifre'
      end
      object Label3: TLabel
        Left = 12
        Top = 77
        Width = 55
        Height = 13
        Caption = 'Tekrar '#350'ifre'
      end
      object txtSIFRE: TEdit
        Left = 90
        Top = 13
        Width = 235
        Height = 21
        PasswordChar = '#'
        TabOrder = 0
        OnKeyPress = txtSIFREKeyPress
      end
      object txtYENISIFRE: TEdit
        Left = 90
        Top = 42
        Width = 235
        Height = 21
        PasswordChar = '#'
        TabOrder = 1
        OnKeyPress = txtSIFREKeyPress
      end
      object txtTEKRARSIFRE: TEdit
        Left = 90
        Top = 72
        Width = 235
        Height = 21
        PasswordChar = '#'
        TabOrder = 2
        OnKeyPress = txtSIFREKeyPress
      end
    end
  end
  object qryKullanici: TIB_Query
    DatabaseName = 'LOCALHOST:D:\Project\DB\MAIN.FDB'
    IB_Connection = DataMod.dbaANA
    SQL.Strings = (
      'SELECT AD_SOY,USER_NAME,PASS_WORD FROM KULLANICI'
      'WHERE KULLANICI_ID=:PRM_KUL_ID '
      'AND KULLANICI_SID=:PRM_KUL_SID')
    ColorScheme = False
    MasterSearchFlags = [msfOpenMasterOnOpen, msfSearchAppliesToMasterOnly]
    BufferSynchroFlags = []
    FetchWholeRows = True
    Left = 132
    Top = 96
  end
  object dtsKullanici: TIB_DataSource
    AutoEdit = False
    AutoInsert = False
    Dataset = qryKullanici
    Left = 210
    Top = 95
  end
end
