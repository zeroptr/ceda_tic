...
type
  TfrmFAT_IRS_SDLG = class(TForm)
    ...
    frxIBODts_FaturaList: TfrxIBODataset;
    btn_Fast: TButton;
...
var
  frmFAT_IRS_SDLG: TfrmFAT_IRS_SDLG;
...
function TfrmFAT_IRS_SDLG.Form_Open(Menu:Boolean;FAT_IRS_SEP:Smallint;var FAT_IRS_ID:Integer;var FAT_IRS_SID:Integer;FAT_IRS_TIP:SmallInt;RAPOR:Boolean):Boolean;
begin
  Application.CreateForm(TfrmFAT_IRS_SDLG, frmFAT_IRS_SDLG);
  with frmFAT_IRS_SDLG do
  begin
...
procedure TfrmFAT_IRS_SDLG.btnCariClick(Sender: TObject);
var
...
procedure TfrmFAT_IRS_SDLG.btn_AraClick(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    1:
      begin
        strSQL := strSQL + ' AND BELGE_NO = '+ SQL_Katar(Fat_No.Text);
      end;
    end;//end case
    if glb_SUBELI then
    begin
  ...
  begin
    ...
    begin
      strSQL := strSQL + ' AND YAZILDI=1 ';
    end;

    if not cb_Yazdirildi.Checked then
    begin
      strSQL := strSQL + ' AND YAZILDI=0 ';
    end;
...
procedure TfrmFAT_IRS_SDLG.btnIPTALClick(Sender: TObject);
begin
  prv_Kontrol := False;
  Close;
end;

procedure TfrmFAT_IRS_SDLG.btnTAMAMClick(Sender: TObject);
begin
...
procedure TfrmFAT_IRS_SDLG.chkCariClick(Sender: TObject);
begin
...
procedure TfrmFAT_IRS_SDLG.Button1Click(Sender: TObject);
var
...
procedure TfrmFAT_IRS_SDLG.Button2Click(Sender: TObject);
var
  Muh_Kod,Aciklama:String;
begin
...
procedure TfrmFAT_IRS_SDLG.Button3Click(Sender: TObject);
var
  Muh_Kod,Aciklama:String;
begin
...
procedure TfrmFAT_IRS_SDLG.Button4Click(Sender: TObject);
var
  Muh_Kod,Aciklama:String;
begin
...
procedure TfrmFAT_IRS_SDLG.Button5Click(Sender: TObject);
var
  Muh_Kod,Aciklama:String;
begin
...
procedure TfrmFAT_IRS_SDLG.txtISLEMMERKEZIExit(Sender: TObject);
begin
  if Length(trim(txtISLEMMERKEZI.Text)) = 0 then
  begin
...
procedure TfrmFAT_IRS_SDLG.txtKOD1Exit(Sender: TObject);
begin
  if Length(trim(txtKOD1.Text)) = 0 then
  begin
    (Sender as TEdit).Color := clWindow;
...
procedure TfrmFAT_IRS_SDLG.txtKOD2Exit(Sender: TObject);
begin
  if Length(trim(txtKOD2.Text)) = 0 then
  begin
    (Sender as TEdit).Color := clWindow;
...
procedure TfrmFAT_IRS_SDLG.txtKOD3Exit(Sender: TObject);
begin
  if Length(trim(txtKOD3.Text)) = 0 then
  begin
    (Sender as TEdit).Color := clWindow;
...
procedure TfrmFAT_IRS_SDLG.txtKOD4Exit(Sender: TObject);
begin
  if Length(trim(txtKOD4.Text)) = 0 then
  begin
    (Sender as TEdit).Color := clWindow;
...
procedure TfrmFAT_IRS_SDLG.btnRaporClick(Sender: TObject);
var
  row,col : integer;
...
begin
  ...
  begin
       ...
       frxReport1.LoadFromFile(glb_REPORTS_DIR+'\Fatura_Listesi.fr3');
...
procedure TfrmFAT_IRS_SDLG.IB_Grid1KeyPress(Sender: TObject; var Key: Char);
begin
  if key =#13 then
  begin
    prv_Kontrol :=True;
...
procedure TfrmFAT_IRS_SDLG.FormCreate(Sender: TObject);
begin
  DataMod.Form_Comp_Color(frmFAT_IRS_SDLG);
  with frmFAT_IRS_SDLG.qryFAT_IRS do
  begin
    FieldByName('GENEL_TOP').DisplayFormat := glb_DOV_FIELD_PF;
  end;
end;

procedure TfrmFAT_IRS_SDLG.txtISLEMMERKEZIEnter(Sender: TObject);
begin
  (Sender as TEdit).Color := glb_Art_Alan_Renk;
end;

procedure TfrmFAT_IRS_SDLG.chkRAPKODEnter(Sender: TObject);
begin
  (Sender as TCheckBox).Color := glb_Art_Alan_Renk;
end;

procedure TfrmFAT_IRS_SDLG.chkRAPKODExit(Sender: TObject);
begin
  (Sender as TCheckBox).Color := clBtnFace;
end;

procedure TfrmFAT_IRS_SDLG.cmbTIPSECEnter(Sender: TObject);
begin
  (Sender as TComboBox).Color := glb_Art_Alan_Renk;
end;

procedure TfrmFAT_IRS_SDLG.cmbTIPSECExit(Sender: TObject);
begin
  (Sender as TComboBox).Color := clWindow;
end;

procedure TfrmFAT_IRS_SDLG.Fat_NoExit(Sender: TObject);
begin
  (Sender as TEdit).Color := clWindow;
end;

procedure TfrmFAT_IRS_SDLG.dtpIslemTarEnter(Sender: TObject);
begin
  (Sender as TDateTimePicker).Color := glb_Art_Alan_Renk;
end;

procedure TfrmFAT_IRS_SDLG.dtpIslemTarExit(Sender: TObject);
begin
  (Sender as TDateTimePicker).Color := clWindow;
end;

procedure TfrmFAT_IRS_SDLG.Tutar1Enter(Sender: TObject);
begin
  (Sender as TIB_Currency).Color := glb_Art_Alan_Renk;
end;

procedure TfrmFAT_IRS_SDLG.Tutar1Exit(Sender: TObject);
begin
  (Sender as TIB_Currency).Color := clWindow;
end;

procedure TfrmFAT_IRS_SDLG.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #27 then
  begin
    btnIPTALClick(self);
...
procedure TfrmFAT_IRS_SDLG.txtKOD4KeyPress(Sender: TObject; var Key: Char);
begin
  if not isDigit(Key) then Key := toUpper(Key);
end;

procedure TfrmFAT_IRS_SDLG.IB_Grid1CellDblClick(Sender: TObject; ACol,ARow: Integer; AButton: TMouseButton; AShift: TShiftState);
begin
//Showmessage(Inttostr(ARow)+'  '+Inttostr(ACol));
   btnTAMAMClick(Self);
end;

procedure TfrmFAT_IRS_SDLG.frxReport1BeforePrint(
