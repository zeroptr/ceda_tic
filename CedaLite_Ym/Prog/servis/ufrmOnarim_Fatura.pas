...
type
  ...
  public
    ...
    function Form_Open(CARI_KOD: String;Cari_Adi:String;Tip:Integer;Dov_Baz_Tar:TDateTime):Boolean;
    { Public declarations }
  end;

var
  frmOnarim_Fatura: TfrmOnarim_Fatura;
...
function TfrmOnarim_Fatura.Form_Open(CARI_KOD:String;Cari_Adi:String;Tip:Integer;Dov_Baz_Tar:TDateTime):Boolean;
begin
  Application.CreateForm (TfrmOnarim_Fatura,frmOnarim_Fatura);
  frmOnarim_Fatura.prv_CARI_KOD:=CARI_KOD;
  frmOnarim_Fatura.lblCariAD.Caption:=Cari_Adi;
  frmOnarim_Fatura.prv_Dov_Baz_Tar:=Dov_Baz_Tar;

  frmOnarim_Fatura.trnFAT_IRS.StartTransaction;
  with frmOnarim_Fatura.qry do
  begin
    Active := False;
    SQL.Clear;
    SQL.Text:=
    ...
    ParamByName('PRM_CARI_KOD').AsString:=frmOnarim_Fatura.prv_CARI_KOD;
    ExecSql;
  end;
  with frmOnarim_Fatura.qry do
  begin
  ...
  frmOnarim_Fatura.trnFAT_IRS.Commit;
  frmOnarim_Fatura.trnFAT_IRS.StartTransaction;
  frmOnarim_Fatura.res_Cari:=TStringList.Create;


  frmOnarim_Fatura.qrySiparis.Active:=False;
  //frmSiparis_Fatura.qrySiparis.ParamByName('PRM_FAT_IRS_SEP').AsInteger:=frmSiparis_Fatura.Prv_FAT_IRS_SEP;
  frmOnarim_Fatura.qrySiparis.ParamByName('PRM_CARI_KOD').AsString:=frmOnarim_Fatura.prv_CARI_KOD;
  frmOnarim_Fatura.qrySiparis.Active:=True;

  if DataMod.FN_CariKOD2Bilgi(frmOnarim_Fatura.prv_Cari_KOD,frmOnarim_Fatura.Res_Cari) then
  frmOnarim_Fatura.lblCariAD.Caption:=frmOnarim_Fatura.Res_Cari[1];

  frmOnarim_Fatura.ShowModal;

  if not frmOnarim_Fatura.prv_Kontrol Then
  begin
    Result := False;
  end
  else
  ...
  frmOnarim_Fatura.Close;
  frmOnarim_Fatura.Release;
end;


procedure TfrmOnarim_Fatura.btnCikisClick(Sender: TObject);
begin
...
procedure TfrmOnarim_Fatura.qrySiparisAfterScroll( IB_Dataset: TIB_Dataset);
begin
...
procedure TfrmOnarim_Fatura.IB_Grid1DblClick(Sender: TObject);
begin
...
procedure TfrmOnarim_Fatura.IB_Grid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   if Key=VK_Space then
      IB_Grid1.OnDblClick(nil);
end;

procedure TfrmOnarim_Fatura.IB_Grid2KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
    if Key=VK_Space then
      IB_Grid2.OnDblClick(nil);
end;

procedure TfrmOnarim_Fatura.SpeedButton1Click(Sender: TObject);
var
   Sonuc:Boolean;
begin
...
procedure TfrmOnarim_Fatura.Aktar(var Kaynak:TIB_Query);
...
procedure TfrmOnarim_Fatura.IB_Grid2DblClick(Sender: TObject);
begin
...
procedure TfrmOnarim_Fatura.IB_Grid1Exit(Sender: TObject);
begin
  if qrySiparis.State in [dssInsert,dssEdit] then
  qrySiparis.Post;
end;

procedure TfrmOnarim_Fatura.qrySiparisAfterPost(IB_Dataset: TIB_Dataset);
begin
  qrySiparisAfterScroll(Nil);
end;

procedure TfrmOnarim_Fatura.IB_Grid2Exit(Sender: TObject);
begin
  if qrySiparis_D.State in [dssInsert,dssEdit] then
  qrySiparis_D.Post;
end;

procedure TfrmOnarim_Fatura.qrySiparis_DCalculateField( Sender: TIB_Statement; ARow: TIB_Row; AField: TIB_Column);
begin
  with AField do
...
procedure TfrmOnarim_Fatura.FormCreate(Sender: TObject);
begin
  DataMod.Form_Comp_Color(frmOnarim_Fatura);
end;
