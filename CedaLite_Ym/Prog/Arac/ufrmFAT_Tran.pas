...
uses
  ...
  IB_Controls, Mask, Grids, IB_Grid, Menus, DB;//kbmMemTable
...
uses main,
     ...
     ufrmSeferSDLG, ufrmSefer, ufrmMazotSDLG, UDm_Fast_Report,ufrmSefer_Takip;
{$R *.dfm}
...
//****************************************************************************//
    ...
    begin
         ...
         Qry_Asl_Urun_Har.Delete;
          ...
          qry_Temp.sql.Clear;
          qry_Temp.sql.Text:='Select * from Sefer Where SATIS_URUN_HAR_ID=:SATIS_URUN_HAR_ID or BEK_FAT_URUN_HAR_ID=:SATIS_URUN_HAR_ID  or SBEK_FAT_URUN_HAR_ID=:SATIS_URUN_HAR_ID or ALIS_URUN_HAR_ID=:SATIS_URUN_HAR_ID';
          ...
          begin
           ...
           //sft
          qry_Temp1.Active:=False;
          qry_Temp1.sql.Clear;
          qry_Temp1.sql.Text:='UPDATE SEFER_TAKIP SET FATURA_NO=NULL WHERE SEFER_TAKIP_ID=:ID';
          qry_Temp1.ParamByName('ID').AsInteger:=qry_Temp.FieldByName('SEFER_TAKIP_ID').AsInteger;
          qry_Temp1.ExecSQL;
               ...
               if qry_Temp.FieldByName('SBEK_FAT_URUN_HAR_ID').AsInteger=StrToInt(Silinen_Idler.Strings[a]) then
               begin
               qry_Temp.FieldByName('SBEK_FAT_URUN_HAR_ID').AsInteger:=0;
               qry_Temp.FieldByName('SBEKLEME_FATURASI_KESILDI').AsInteger:=0;
    ...
    begin
         ...
         begin
            ...
            Qry_Asl_Urun_Har.FieldByName('SSEFER_BEKLEME').AsVariant:=qryURUNHAR.FieldByName('SSEFER_BEKLEME').AsVariant;
...
//****************************************************************************//
          ...
          if MainForm.FindChildFrm1(Application, 'frmSefer_Takip') then
          begin
              //frmSefer_Takip.cxG1DBTV1.DataController.DataSet.Refresh;
              //frmSefer_Takip.qry_Sefer_Takip.Refresh;
              frmSefer_Takip.cxG1DBTV1.NavigatorButtons.Refresh.Click;
          end;
...
begin
  ...
  if IB_Dataset.FieldByName('ACIKLAMA').AsString = ''  then
  begin
  ...
  begin
    ...
    begin
         ...
         begin
          ...
          qry_Temp1.Active:=False;
          qry_Temp1.sql.Clear;
          qry_Temp1.sql.Text:='UPDATE SEFER_TAKIP SET FATURA_NO=:FATURA_NO WHERE SEFER_TAKIP_ID=:ID';
          qry_Temp1.ParamByName('ID').AsInteger:=QryDegTrans.FieldByName('SEFER_TAKIP_ID').AsInteger;
          qry_Temp1.ParamByName('FATURA_NO').AsString:=qryFAT_IRS.FieldByName('BELGE_NO').AsString;
          qry_Temp1.ExecSQL;
...
//y
              if QryDegTrans.FieldByName('SBEKLEME_FAT_VAR').AsInteger=1 then
              begin
                   if (Giden_Idler_Bekleme.indexOf(Inttostr(QryDegTrans.FieldByName('SEFER_ID').AsInteger))=-1)
                   and
                  (QryDegTrans.FieldByName('SBEKLEME_FATURASI_KESILDI').AsInteger =0)
                  ...
                  then
                   begin
                     ...
                     //txtUrunKoduExit(Self);
                     qryURUNHAR.FieldByName('MIKTAR').AsFloat:=1;
                     qryURUNHAR.FieldByName('BIRIM_FIY').AsFloat:=QryDegTrans.FieldByName('SBEKLEME_FAT_TUTAR').AsFloat;
                     qryURUNHAR.FieldByName('DOV_BAZ_TAR').AsDateTime:=QryDegTrans.FieldByName('SBEKLEME_FAT_DOV_BAZ_TAR').AsDateTime;
                     qryURUNHAR.FieldByName('DOVKOD').AsString:=QryDegTrans.FieldByName('SBEKLEME_FAT_DOV_KOD').AsString;
                     qryURUNHAR.FieldByName('DOVKUR').AsFloat:=QryDegTrans.FieldByName('SBEKLEME_FAT_DOVKUR').AsFloat;
                     qryURUNHAR.FieldByName('TUTAR').AsFloat:=QryDegTrans.FieldByName('SBEKLEME_FAT_TUTAR').AsFloat;
                     qryURUNHAR.FieldByName('TUTAR_VPB').AsFloat:=QryDegTrans.FieldByName('SBEKLEME_FAT_VPB').AsFloat;
                     qryURUNHAR.FieldByName('TARIH').AsDate := QryDegTrans.FieldByName('BIT_TAR').AsDate;
                     qryURUNHAR.FieldByName('SECIM').AsString:='X';
                     qryURUNHAR.FieldByName('ACIKLAMA').AsString:=
                     QryDegTrans.FieldByName('CIK_YER').AsString+ ' '+
                     ...
                     QryDegTrans.Edit;
                     QryDegTrans.FieldByName('SBEK_FAT_URUN_HAR_ID').AsInteger:=qryURUNHAR.FieldByName('URUNHAR_ID').AsInteger;
                     QryDegTrans.Post;
...
begin
  ...
  begin
       ...
       qry_Temp.Active:=False;
       qry_Temp.sql.Clear;
       qry_Temp.sql.Text:='Select * from Sefer Where ALIS_URUN_HAR_ID=:ALIS_URUN_HAR_ID OR SBEK_FAT_URUN_HAR_ID=:ALIS_URUN_HAR_ID';
       qry_Temp.ParamByName('ALIS_URUN_HAR_ID').AsInteger:=Gelen_Id;
       qry_Temp.Active:=True;
       if qry_Temp.FieldByName('SEFER_ID').aSInteger>0 then
       begin
...
//Y
          ...
          begin
               ...
               begin
                    ...
                    if qry_Temp.FieldbyName('SBEKLEME_FAT_VAR').AsInteger=1 then
                    CARI_SIL:=0
                    else
                    CARI_SIL:=1;
                    ...
                    if qry_Temp.FieldbyName('SBEKLEME_FAT_VAR').AsInteger=1 then
                    begin
                      if not DataMod.SALICI_BEKLEME_CARI_HAR(CARI_SIL,ALACAK,PRV_ISN,BT_SEFER,qry_Temp,trnFAT_IRS) then
                      begin
                        frmISN_ERRORS.AnyErr(trnFAT_IRS,PRV_ISN,0);
                        Result := False;
                        Exit;
...
begin
  ...
  begin
    ...
    if qryURUNHAR.FieldByName('SSEFER_BEKLEME').AsInteger=1 then
    if DataMod.Urun_Kod_Karsilastir(qryURUNHAR.FieldByName('URUN_KOD').AsString,glb_BEK_HIZ_URN_KOD) then
    Sefer_Mazot_Id_Duzenle('SEFER','SIL',True,qryURUNHAR.FieldByName('URUNHAR_ID').AsInteger,qryURUNHAR.FieldByName('SEFER_ID').AsInteger);
...
//       qry_Temp.sql.Text:='Select * from SEFER Where ALIS_URUN_HAR_ID=:ALIS_URUN_HAR_ID or SATIS_URUN_HAR_ID=:ALIS_URUN_HAR_ID or BEK_FAT_URUN_HAR_ID=:ALIS_URUN_HAR_ID';
       qry_Temp.sql.Text:='Select * from SEFER Where ALIS_URUN_HAR_ID=:ALIS_URUN_HAR_ID or SATIS_URUN_HAR_ID=:ALIS_URUN_HAR_ID or BEK_FAT_URUN_HAR_ID=:ALIS_URUN_HAR_ID or SBEK_FAT_URUN_HAR_ID=:ALIS_URUN_HAR_ID';
       qry_Temp.ParamByName('ALIS_URUN_HAR_ID').AsInteger:=qryURUNHAR.FieldByName('URUNHAR_ID').AsInteger;
       qry_Temp.Active:=True;
       While not qry_Temp.Eof do
       begin
         ...
         begin
               ...
               begin
                  ...
                  begin
                      ...
                      if DataMod.Urun_Kod_Karsilastir(qryURUNHAR.FieldByName('URUN_KOD').AsString,glb_BEK_HIZ_URN_KOD) then
                      if qry_Temp.FieldByName('SBEKLEME_FATURASI_KESILDI').AsInteger=1 then
                      begin
                          Gelen_Idler_Bekleme.Add(Inttostr(qryURUNHAR.FieldByName('URUNHAR_ID').AsInteger));
                          Giden_Idler_Bekleme.Add(Inttostr(qry_Temp.FieldByName('SEFER_ID').AsInteger));
                      end;
       ...
       begin
          ...
          begin
             ...
             begin
               ...
               qry_Temp.Edit;
               qry_Temp.FieldByName('SBEKLEME_FATURASI_KESILDI').AsInteger:=1;
               qry_Temp.FieldByName('SBEK_FAT_URUN_HAR_ID').AsInteger:= Qry_Asl_Urun_Har.FieldByName('URUNHAR_ID').AsInteger;
               qry_Temp.Post;

               if not DataMod.SALICI_BEKLEME_CARI_HAR(1,ALACAK,PRV_ISN,BT_SEFER,qry_Temp,trnFAT_IRS) then
               begin
                 frmISN_ERRORS.AnyErr(trnFAT_IRS,PRV_ISN,0);
                 Result := False;
                 Exit;
     ...
     begin
       ...
       begin
          ...
          begin
             qry_Temp.Active:=False;
             qry_Temp.sql.Clear;
             qry_Temp.sql.Text:='Select * from Sefer Where SBEK_FAT_URUN_HAR_ID=:BEK_FAT_URUN_HAR_ID or SEFER_ID=:SEFER_ID';
             qry_Temp.ParamByName('BEK_FAT_URUN_HAR_ID').AsInteger:=Qry_Asl_Urun_Har.FieldByName('URUNHAR_ID').AsInteger;
             qry_Temp.ParamByName('SEFER_ID').AsInteger:=Qry_Asl_Urun_Har.FieldByName('SEFER_ID').AsInteger;
             qry_Temp.Active:=True;
