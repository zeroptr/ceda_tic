...
type
  TfrmSefer = class(TForm)
    ...
    GroupBox4: TGroupBox;
    IB_Currency16: TIB_Currency;
    IB_Currency17: TIB_Currency;
    cboDOVKOD3: TIB_ComboBox;
    IB_Date9: TIB_Date;
    IB_Text14: TIB_Text;
    Panel5: TPanel;
    Cb_Yukleyici_Firma: TIB_ComboBox;
    Btn_Sefer_Takip_sec: TButton;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    procedure IB_CheckBox8Click(Sender: TObject);
    procedure FormResize(Sender: TObject);
    procedure FormActivate(Sender: TObject);
    procedure Btn_Sefer_Takip_secClick(Sender: TObject);
    procedure qrySeferBeforeCancel(IB_Dataset: TIB_Dataset);
////////////////////////////////////////////////////////////////////////////////
  private
...
uses unDATAMOD, main, ufrmISN_ERRORS, ufrmCariSDLG,unFUNC,  ufrmKasaSDGL, unYazici, unLogger,
  ufrmSeferSDLG,ufrmSefer_Takip,ufrmSefer_TakipSDLG;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        While not qryDeg.Eof do
        begin
            Cb_Tas_Esya.Items.Add(qryDeg.FieldByName('TASINAN_ESYA').asString);
            qryDeg.Next;
        end;
        qryDeg.Active:=False;
        ...
        While not qryDeg.Eof do
        begin
            Cb_Yukleyici_Firma.Items.Add(qryDeg.FieldByName('YUKLEYICI_FIRMA').asString);
            qryDeg.Next;
        end;
        qryDeg.Active:=False;
    ...
    try
      ...
      0:
        begin
          ...
          Btn_Sefer_Takip_sec.Visible:=False;
        end;
      1:
        begin
          with qrySefer do
...
////////////////////////////////////////////////////////////////////////////////
          ...
          Btn_Sefer_Takip_sec.Visible:=True;
...
begin
  ...
  with qrySefer do
  begin
    PRV_SAYAC_NO:=   DataMod.GET_SAYAC_NUM('SEFER',True,True,0);
    FieldByName('SEFER_ID').AsInteger := PRV_SAYAC_NO;//qrySefer.Gen_ID('GEN_SEFER',1);


    if FieldByName('SEFER_ID').AsInteger=1999999 then
    begin
    PRV_SAYAC_NO:=   DataMod.GET_SAYAC_NUM('SEFER',True,True,0);
    FieldByName('SEFER_ID').AsInteger := PRV_SAYAC_NO;
    //DataMod.GET_SAYAC_NUM('SEFER',True,True,0);
...
procedure TfrmSefer.FormActivate(Sender: TObject);
begin
  frmSefer.Top:=0;
end;
...
begin
    {if MainForm.FindChildFrm(Application, 'frmSefer_Takip') then
    begin
        Application.MessageBox('Sefer takip formu açýk. Ýþlemden Vazgeçildi','Dikkat',MB_ICONWARNING);
        Exit;
    end;}
...
procedure TfrmSefer.FormResize(Sender: TObject);
begin
  frmSefer.Top:=0;
end;
...
begin
  ...
  begin
    ...
    //frmSefer_Takip.cxG1DBTV1.DataController.Refresh;
  end;
end;
...
begin
  ...
  try
     ...
     qryDeg.Active:=False;
     qryDeg.SQL.Clear;
     qryDeg.SQL.Text:='UPDATE SEFER_TAKIP SET SEFER_ID=0, SEFERLESTI=0 WHERE SEFER_TAKIP_ID=:SEFER_TAKIP_ID AND SEFER_TAKIP_SID=:SEFER_TAKIP_SID';
     qryDeg.ParamByName('SEFER_TAKIP_ID').AsInteger:=qrySefer.FieldbyName('SEFER_TAKIP_ID').AsInteger;
     qryDeg.ParamByName('SEFER_TAKIP_SID').AsInteger:=qrySefer.FieldbyName('SEFER_SID').AsInteger;
     qryDeg.ExecSQL;                                                                                        ;
...
begin
  ...
  cboDOVKOD3.Items.Assign(glb_DOV_LIST);
...
procedure TfrmSefer.qrySeferBeforeCancel(IB_Dataset: TIB_Dataset);
begin
  if dtsSefer.State = dssINSERT then
  begin
    DataMod.Add_Sayaclar_Nu('SEFER',PRV_SAYAC_NO,0);
  end;
end;
...
procedure TfrmSefer.IB_CheckBox8Click(Sender: TObject);
begin
     if sBekleme_Ilk_Giris then
     begin
          if qrySefer.FieldByName('SBEKLEME_FAT_VAR').AsInteger=1 then
          IB_Currency16.TabStop:=False
          else
...
begin
     ...
     begin
     Prv_Id_Os:=qrySefer.FieldByName('SEFER_ID').AsInteger;
     Prv_Idd_Os:=qrySefer.FieldByName('SEFER_SID').AsInteger;
     end;
     ...
     qryDeg1.ParamByName('SEFER_ID').AsInteger:=Prv_Id_Os;
     //qrySefer.FieldByName('SEFER_ID').AsInteger;
     qryDeg1.ParamByName('SEFER_SID').AsInteger:=Prv_Idd_Os;
     //qrySefer.FieldByName('SEFER_SID').AsInteger;
     qryDeg1.Open;
...
procedure TfrmSefer.Btn_Sefer_Takip_secClick(Sender: TObject);
var
   Id,Sid:Integer;
begin

  if frmSefer_TakipSDLG.Form_Open(False,Id,Sid) then
  begin
    //
      if not (qrySefer.State in [dssInsert,dssEdit]) then
        qrySefer.Edit;
        qrySefer.FieldByName('SEFER_TAKIP_ID').AsInteger:= Id;
        //qrySefer.Post;
