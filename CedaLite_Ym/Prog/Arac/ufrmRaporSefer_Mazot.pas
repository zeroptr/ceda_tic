...
type
  TfrmRaporSefer_Mazot = class(TForm)
    Label36: TLabel;
    TxtMazotcu: TEdit;
    Button7: TButton;
    Label19: TLabel;
...
var
  frmRaporSefer_Mazot: TfrmRaporSefer_Mazot;
...
procedure TfrmRaporSefer_Mazot.Button7Click(Sender: TObject);
...
procedure TfrmRaporSefer_Mazot.TxtMazotcuExit(Sender: TObject);
var
  CARI_AD:String;
begin
...
procedure TfrmRaporSefer_Mazot.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TfrmRaporSefer_Mazot.Button1Click(Sender: TObject);
begin
     Close;
end;

procedure TfrmRaporSefer_Mazot.Btn_BulClick(Sender: TObject);
var
   ilk_sql:AnsiString;
...
//   ' INNER JOIN MAZOT_FISI M ON (MAZOT_FISI.ALIS_URUN_HAR_ID=SEFER.SEFER_ID) '+
   ' INNER JOIN MAZOT_FISI M ON (MAZOT_FISI.SEFER_ID=SEFER.SEFER_ID) '+
   ...
   begin
       ilk_sql_w:=ilk_sql_w+
       ' AND MAZOT_FISI.ALAN_CARI_KOD='+Sql_Katar(Trim(TxtMazotcu.Text))+' ';
       Ek_Var:=True;
   end;
   ...
   begin
       ...
       ilk_sql_w:=ilk_sql_w+
       ' AND MAZOT_FISI.ALAN_PERSONEL_KOD='+Sql_Katar(Trim(txtAlanPers.Text))+' ';
       Ek_Var:=True;
   end;
   ...
   begin
       //if Ek_Var then ilk_sql_w:=ilk_sql_w+ ' AND ';
       ilk_sql_w:=ilk_sql_w+
       ' AND MAZOT_FISI.ARAC_KOD='+Sql_Katar(Trim(txtArac.Text))+' ';
       Ek_Var:=True;
   end;
   ...
   begin
       //if Ek_Var then ilk_sql_w:=ilk_sql_w+ ' AND ';
       ilk_sql_w:=ilk_sql_w+
       ' AND MAZOT_FISI.MAZOTCU_CARI_KOD='+Sql_Katar(Trim(Edit1.Text))+' ';
       Ek_Var:=True;
   end;
...
procedure TfrmRaporSefer_Mazot.Button2Click(Sender: TObject);
begin
...
procedure TfrmRaporSefer_Mazot.Button6Click(Sender: TObject);
var
 ARAC_KOD:String;
 PLAKA:String;
 CARI_KOD:String;
...
procedure TfrmRaporSefer_Mazot.txtAracExit(Sender: TObject);
var
 ARAC_KOD:String;
 PLAKA:String;
 SATICI_KOD:String;
...
procedure TfrmRaporSefer_Mazot.txtAlanPersExit(Sender: TObject);
var
  PERSONEL_AD:String;
begin
...
procedure TfrmRaporSefer_Mazot.Button8Click(Sender: TObject);
var
...
procedure TfrmRaporSefer_Mazot.txtAracKeyPress(Sender: TObject; var Key: Char);
begin
  if not isDigit(Key) Then Key := toUpper(Key);
end;

procedure TfrmRaporSefer_Mazot.Button3Click(Sender: TObject);
begin
...
procedure TfrmRaporSefer_Mazot.Edit1Exit(Sender: TObject);
var
  CARI_AD:String;
begin
...
procedure TfrmRaporSefer_Mazot.Button4Click(Sender: TObject);
var
  Cari_KOD,Cari_AD:String;
begin
...
procedure TfrmRaporSefer_Mazot.txtAracEnter(Sender: TObject);
begin
