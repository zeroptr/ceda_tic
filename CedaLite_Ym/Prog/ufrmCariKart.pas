...
////////////////////////////////////////////////////////////////////////////////
    ...
    procedure dts_FiyCariStateChanged(Sender: TIB_DataSource;
  ...
  2://cari gruplarý
   ...
   4: // fiyatlandýrma
     begin
       ...
       begin
         Active := False;
         ParamByName('PRM_CARI_KOD').AsString := IB_Edit1.Text;
         Active := True;
       end;
     end;
  end;
...
procedure TfrmCariKart.btnCariFiyDegClick(Sender: TObject);
begin
  qry_FiyCari.Edit;
end;

procedure TfrmCariKart.btnCariFiyEkleClick(Sender: TObject);
var
...
begin
  ...
  begin
    qry_FiyCari.FieldByName('URUN_KOD').AsString := URUN_KOD;
    qry_FiyCari.FieldByName('URUN_AD').AsString := URUN_AD;
  end;
end;

procedure TfrmCariKart.btnCariFiyIptalClick(Sender: TObject);
begin
  qry_FiyCari.Cancel;
  dmLOGGER.dbaMain_CancelMonitoring;
end;

procedure TfrmCariKart.btnCariFiyKaydetClick(Sender: TObject);
begin
  try
...
procedure TfrmCariKart.btnCariFiySilClick(Sender: TObject);
begin
  try
    if dmLOGGER.dbaMain_StartMonitoring then
    begin
...
procedure TfrmCariKart.dts_FiyCariStateChanged(Sender: TIB_DataSource;
  ADataset: TIB_Dataset);
begin
...
procedure TfrmCariKart.qry_FiyCariAfterInsert(IB_Dataset: TIB_Dataset);
begin
  with qry_FiyCari do
  begin
    FieldByName('CARI_KOD').AsString := qryCari.FieldByName('CARI_KOD').AsString;
    FieldByName('SATISDOVKOD').AsString:= glb_DEFCUR;
...
procedure TfrmCariKart.qry_FiyCariBeforeDelete(IB_Dataset: TIB_Dataset);
begin
...
procedure TfrmCariKart.qry_FiyCariBeforePost(IB_Dataset: TIB_Dataset);
var
  strSQL:String;
begin
  if qry_FiyCari.State in [dssInsert] then
  begin
    strSQL := 'SELECT * FROM URUN_CARI_FIY WHERE URUN_KOD= ' + SQL_Katar(qry_FiyCari.FieldByName('URUN_KOD').AsString)
    ...
    begin
      ...
      begin
        Application.MessageBox('Ürün Daha Önceden Eklenmiþ','Dikkat',MB_ICONWARNING);
        Abort;
      end;
    end;
  end
