...
uses
  SysUtils, Classes, DB, IBODataset, GenTypes,IB_Components,
  Forms,Windows,Printers,Messages,DateUtils, Dialogs,Winspool, frxClass;
//pr_Common, pr_TxClasses,pr_TxDesigner,pr_TxConsts, pr_Utils,pr_Classes pr_Parser
type
  TYazici = class(TDataModule)
    ...
    rptYazdir: TfrxReport;
    qryFAT_IRS_ACK: TIBOQuery;
//****************************************************************************//
...
{    function GET_DSGN_SETTINGS_MODAL(PRM_BELGE_TUR:Integer;PRM_SIRA_NO:Byte;ReportObj:TprTxReport):Boolean;
    ...
    function GET_DSGN_SETTINGS_MODAL(PRM_BELGE_TUR:Integer;PRM_SIRA_NO:Byte;ReportObj:TFrxReport):Boolean;
    function GET_DSGN_SETTINGS(PRM_BELGE_TUR:Integer;PRM_SIRA_NO:Byte;ReportObj:TFrxReport):Boolean;
    function GET_DSGN_SETTINGS_A4(PRM_BELGE_TUR:Integer;PRM_SIRA_NO:Byte;ReportObj:TFrxReport):Boolean;
...
//    procedure rptYazdirUnknownVariable(Sender: TObject; const VarName: String; var Value: TprVarValue; var IsProcessed: Boolean);
    ...
    procedure rptYazdirAfterPrintReport(Sender: TObject);
...
uses unDataMod,unFunc,unRegistry,unCEKSENHAR,unLogger, {$IfDef TRANSPORT} ufrmFAT_Tran,  {$EndIf}
  ufrmFAT_IRS, ufrmSiparis;
...
begin
  ...
  qryFAT_IRS_ACK.ParamByName('PRM_FAT_IRS_SEP').AsInteger := sep;
  qryFAT_IRS_ACK.ParamByName('PRM_FAT_IRS_ID').AsInteger := ID;
  qryFAT_IRS_ACK.ParamByName('PRM_FAT_IRS_SID').AsInteger := SID;
  qryFAT_IRS_ACK.Open;
  ...
  begin
    SQL.Clear;
    SQL.Add('SELECT * FROM URUNHAR WHERE FATURA_ID='+inttostr(ID)+' AND FATURA_SID ='+inttostr(SID)+' ORDER BY IRSALIYE_ID,URUNHAR_ID');
  end;
  end;
  ...
  begin
    SQL.Clear;
    SQL.Add('SELECT * FROM URUNHAR WHERE IRSALIYE_ID='+inttostr(ID)+' AND IRSALIYE_SID ='+inttostr(SID)+' ORDER BY IRSALIYE_ID,URUNHAR_ID');
  end;
  end;
...
function TYazici.GET_DSGN_SETTINGS_A4(PRM_BELGE_TUR:Integer;PRM_SIRA_NO:Byte;ReportObj:TFrxReport):Boolean;
var
  ...
  try
    try
      ...
      begin
        ...
        ReportObj.LoadFromFile(Dosya_Ad,false);

        //ReportObj.PrinterName := GetBelgePrinter(PRM_BELGE_TUR,PRM_SIRA_NO);
        ReportObj.PrintOptions.Printer := GetBelgePrinter(PRM_BELGE_TUR,PRM_SIRA_NO);
        qryBELGE_YAZ.Close;
        Result := True;
      end
      else
...
function TYazici.GET_DSGN_SETTINGS(PRM_BELGE_TUR:Integer;PRM_SIRA_NO:Byte;ReportObj:TFrxReport):Boolean;
var
  Dosya_Ad:String;
  qryBELGE_YAZ:TIB_Query;
  Yazici_Is:Integer;
  ...
  try
    try
      ...
      begin
        ...
        Dosya_Ad:=glb_REPORTS_DIR+'\'+ExtractFilename(qryBELGE_YAZ.FieldByName('DOSYA_PATH').AsString);
        if not FileExists(Dosya_Ad) then
        begin
          Application.MessageBox(PChar(dosya_Ad + ' Dosyasýna ulaþýlamadý!'),'Dikkat',MB_ICONERROR);
          Result := False;
          Exit;
        end;
...
//            Yazici_Is:=Yazici.get_is(ReportObj.PrinterName);
            ...
            Yazici_Is:=Yazici.get_is(ReportObj.PrintOptions.Printer);
            if Yazici_Is>0 then
            begin
...
function TYazici.GET_DSGN_SETTINGS_MODAL(PRM_BELGE_TUR: Integer; PRM_SIRA_NO: Byte; ReportObj: TFrxReport): Boolean;
var
  qryBELGE_YAZ:TIB_Query;
  Dosya_Ad:String;
begin
...
procedure TYazici.rptYazdirAfterPrintReport(Sender: TObject);
var
   qryDeg:TIb_Query;
begin
