...
begin
  ...
  begin
    ...
    begin
        ...
        begin
           ...
           begin
             Hata;
             ShowMessage('Dosya Kopyalanamadý: '+Search.Name);
             Result:=False;
           end
           else
           Inc(Say);
      ...
      //ShowMessage('Hedef Dizin Bulunamadý');
    end
    else
    begin
      {//alt dizin kopyalama
  ...
  begin
     Hata;
     ShowMessage('Hiç Dosya Kopyalanamadý: '+ Search.Name);
     Result:=False;
  end;
...
////////////////main kontrol //////////////////
  ...
  begin
      Application.MessageBox(Pchar(dos_ad+' Açýk Data var Ýþlem Yapýlmayacak, Açýk Programlarý Kapatýnýz'),'Bilgi',MB_OK);
      //Exit;
      Halt;
  end;
...
begin
  ...
  begin
    ...
    Lbl_Durum.Caption:='Þu An Laptopdasýnýz';
    Lbl_Durum.Update;
  end
  else
  begin
    ...
    Lbl_Durum.Caption:='Þu An Ofistesiniz';
    Lbl_Durum.Update;
  end;
...
Screen.Cursor:=CrHourGlass;
  ...
  begin
     ...
     begin
          ...
          begin
            Hata;
            ShowMessage('Host ini Dosyasi Kopyalanamadý ');
            Application.MessageBox('Aktarmada Hatalar Oluþtu Yeniden Deneyiniz','Bilgi',MB_OK);
          ...
          Buton_Yenile(1);
          Application.MessageBox('Aktarma Tamamlandý','Bilgi',MB_OK);
          Screen.Cursor:=CrDefault;
          Exit;
     ...
     begin
          ...
          begin
            Hata;
            ShowMessage('Host ini Dosyasý Kopyalanamadý ');
            Application.MessageBox('Aktarmada Hatalar Oluþtu Yeniden Deneyiniz','Bilgi',MB_OK);
            Screen.Cursor:=CrDefault;
            Exit;
          end;
 ...
 //
  if Sender =  Btn_Laptop_Ofis then
  if Application.MessageBox('Ana Makinaya Datalarý Aktarmak istiyormusunuz?','Dikkat',MB_YESNO + MB_DEFBUTTON2+ MB_ICONQUESTION) = IDNo then ABORT;
...
begin
//  if Od.Execute then
  txtYerel.Text:=BrowseDialog('Yerel Data Dizini Seçiniz',BIF_RETURNONLYFSDIRS);
end;

procedure TForm1.Button2Click(Sender: TObject);
begin
//  if Od.Execute then
  txtAg.Text:=BrowseDialog('Yerel Data Dizini Seçiniz',BIF_RETURNONLYFSDIRS);
end;
...
begin
     ...
     begin
          ...
          begin
              Result:=False;
              hata;
              Application.MessageBox('Açýk Data var Ýþlem Yapýlmayacak, Açýk Programlarý Kapatýnýz','Bilgi',MB_OK);
              Exit;
          end;
...

    {
       ...
       Application.MessageBox('Dosya Listesi Alýnamadý','Bilgi',MB_OK);
       Hata;
       Result:=False;
    end;
end;
